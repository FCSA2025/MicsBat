# Documented File: TestFtPrint.cs
**Repository Path:** `Tools\TestFtPrint.cs`
**Primary Layer:** `Tools`
**Namespace:** `Tools`
...
namespace Tools
{
    ...
    public class TestFtPrint
    {
        ...
        //const string NATIVE_BIN_DIR = @"D:\prod\bin";
        const string NATIVE_BIN_DIR = @"D:\Users\ahulme\FCSA_Native_Progs\bin20180406";
        const string FTIMPORT_NATIVE_EXE = NATIVE_BIN_DIR + @"\FtImport.exe";
        const string FTPRINT_NATIVE_EXE = NATIVE_BIN_DIR + @"\FtPrint.exe";
        const string FTIMPORT_MICS_EXE = MICS_RELEASE_BIN_DIR + @"\FtImport.exe";
        const string FTPRINT_MICS_EXE = MICS_RELEASE_BIN_DIR + @"\FtPrint.exe";
        const string COMPFXVALIDATEOUTPUTS_EXE = AHULME_BIN_DIR + @"\CompFxValidateOutputs.exe";
        const string TS_NATIVE_OUT_DIR = @"D:\Users\ahulme\MICS#\TestData\TS\ProdBin_Results";
        ...
        private static bool mRunNative = false;
        ...
        private static int mNativeExitCode;
        ...
        public static void Test_FtPrint(string[] args)
        {
            string stdOut;
            string stdErr;
            int exitCode;
            ...
            try
            {
                ...
                string mLog2FilePath = @"d:\MicsBatchLogs\Test_FtPrint.log";
                ...
                for (int i = 0; i < mPdfNames.Length; i++)
                {
                    ...
                    string commandLineNativeFtImportArgs = CreateCommandLineArgsForNativeFtImport(pdfName, pdfPath);
                    ...
                    string nativeFtStdOutFilePath = TS_NATIVE_OUT_DIR + @"\ProdBin_FtImport_" + pdfName + ".out";
                    string micsFtStdOutFilePath = TS_MICS_CS_OUT_DIR + @"\MICS#_FtImport_" + pdfName + ".out";

                    // Construct the path of the file that FtPrint's stdOut is written to.
                    string nativeFtPrintFilePath = TS_NATIVE_OUT_DIR + @"\ProdBin_FtImport_" + pdfName + ".pri";
                    string micsFtPrintFilePath = TS_MICS_CS_OUT_DIR + @"\MICS#_FtImport_" + pdfName + ".pri";

                    // ftPrint: Native Code Program -------------------------------------------------------------------------------
                    if (mRunNative)
                    {
                        // Now execute FtImport as a command-line task.
                        Run(FTIMPORT_NATIVE_EXE, commandLineNativeFtImportArgs, out stdOut, out stdErr, out exitCode);
                        // Write the stdOut of FtImport.exe to file.
                        File.WriteAllText(nativeFtStdOutFilePath, stdOut);
                        // Now run FtPrint to see how FtImport changed the DB.
                        Run(FTPRINT_NATIVE_EXE, commandLineFtPrintArgs, out stdOut, out stdErr, out mNativeExitCode);
                        // Write the stdOut of FtPrint.exe to file.
                        File.WriteAllText(nativeFtPrintFilePath, stdOut);
                    }

                    // FtPrint: MICS C# Program -----------------------------------------------------------------------------------
                    if (mRunMicsCs)
                    {
                        // Now execute FtImport as a command-line task.
                        Run(FTIMPORT_NATIVE_EXE, commandLineNativeFtImportArgs, out stdOut, out stdErr, out exitCode);
                        ...
                        // Now run FtPrint to see how FtImport changed the DB.
                        Run(FTPRINT_MICS_EXE, commandLineFtPrintArgs, out stdOut, out stdErr, out mMicsExitCode);
                        // Write the stdOut of FtPrint.exe to file.
                        File.WriteAllText(micsFtPrintFilePath, stdOut);
                    }

                    // We can use CompFtValOut.exe to compare the .pri output files.
                    if (mRunNative && mRunMicsCs)
                    {
                        string errorIndicator = "";

                        Run(COMPFXVALIDATEOUTPUTS_EXE, "  " + TS_NATIVE_OUT_DIR + @"\ProdBin_FtImport_" + pdfName + ".out  " + TS_MICS_CS_OUT_DIR + @"\MICS#_FtImport_" + pdfName + ".out", out stdOut, out stdErr, out exitCode);
                        Console.Write("\n" + stdOut);

                        string exitCodeResult = "PASS";
                        if (mNativeExitCode != mMicsExitCode)
                        {
                            exitCodeResult = "----";
                            errorIndicator = "<<<<<<<<<<";
                        }
                        ...
                        Run(COMPFXVALIDATEOUTPUTS_EXE, "  " + nativeFtPrintFilePath + "  " + micsFtPrintFilePath, out stdOut, out stdErr, out exitCode);
                        Console.Write("\n" + stdOut);
                        ...
                        result = String.Format("{0,3}  {1} / {2} / {3}     {4}", mNativeExitCode, exitCodeResult, stdOutResult, ftPrintResult, errorIndicator);
                    }
        ...
        public static void WriteCliUsageMessage()
        {
            ...
            const string TAB2 = "        ";
            StringBuilder sb = new StringBuilder("\r\n\r\n");
            sb.AppendLine(TAB1 + "Usage:    testing FtPrint [-v] [-m] [-p] [-o:PDFname] <-f:PDFlistFilePath>");
            sb.AppendLine();
            ...
            sb.AppendLine();
            sb.AppendLine("    <...> indicates a mandatory argument.");
            sb.AppendLine("    [...] indicates an optional argument.");
            sb.AppendLine();
        ...
        /// <param name="pdfPath"></param>
        /// <returns></returns>
        private static string CreateCommandLineArgsForNativeFtImport(string pdfName, string pdfPath)
        {
