# Documented File: TpReport.cs
**Repository Path:** `_Utillib\TpReport.cs`
...
namespace _Utillib
{
    ...
    public class TpReport
    {
        ...
        public static int CreateTTStatRep(string proposed, string siteName, out string cStatTab)
        {
            ...
            +
            ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
            GenUtil.MkUnique(out cStatTab, Info.GlobalSchema, "", "tsip_stat", "", "");

            string x_cStatTab, y_cStatTab;
            GenUtil.MkUnique(out x_cStatTab, Info.GlobalSchema, "x_", "tsip_stat", "", "");
            GenUtil.MkUnique(out y_cStatTab, Info.GlobalSchema, "y_", "tsip_stat", "", "");
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\nTpReport.CreateTTStatRep(): ERROR: A. Could not create temporary table: {0}\nSQL query = {1}", cStatTab, sqlstmt);
                Log2.e("\n" + ODBC.GetDiagnostics(hStmt, sqlstmt));
            ...
            /*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            + We first create and populate the temporary table with victim call1 information.
            +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
            ...
            {
                Log2.e("\nTpReport.CreateTTStatRep(): ERROR: B. Could not insert into temporary table: " + cStatTab);
                Ssutil.DbGetDiagStmt(hStmt, "createTTStatRep02: Could not insert into temporary table: " + cStatTab);
                ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                Ssutil.DisConn(hConn);
                return -2;
            }
            ...
            if (!ODBC.IsOK(sqlRet) && !ODBC.IsNoData(sqlRet))
            {
                Log2.e("\nTpReport.CreateTTStatRep(): ERROR: C. Could not insert into temporary table: " + cStatTab);
                Ssutil.DbGetDiagStmt(hStmt, "createTTStatRep03: Could not insert into temporary table: " + cStatTab);
                ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                Ssutil.DisConn(hConn);
                return -3;
            }
            ...
            if (!ODBC.IsOK(sqlRet) && !ODBC.IsNoData(sqlRet))
            {
                Log2.e("\nTpReport.CreateTTStatRep(): ERROR: D. Could not insert into temporary table: " + cStatTab);
                Ssutil.DbGetDiagStmt(hStmt, "createTTStatRep04: Could not insert into temporary table: " + cStatTab);
                ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                Ssutil.DisConn(hConn);
                return -4;
            }
            ...
            if (!ODBC.IsOK(sqlRet) && !ODBC.IsNoData(sqlRet))
            {
                Log2.e("\nTpReport.CreateTTStatRep(): ERROR: E. Could not create temporary table: " + x_cStatTab);
                Log2.e("\n" + ODBC.GetDiagnostics(hStmt, sqlstmt));
                Log2.e("\nInfo.GlobalSchema = {0}", Info.GlobalSchema);
                Ssutil.DbGetDiagStmt(hStmt, "createTTStatRep05: Could not create temporary table: " + x_cStatTab);
                ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                Ssutil.DisConn(hConn);
                return -5;
            }
            ...
            if (!ODBC.IsOK(sqlRet) && !ODBC.IsNoData(sqlRet))
            {
                Log2.e("\nTpReport.CreateTTStatRep(): ERROR: F. Could not create temporary table: " + y_cStatTab);
                Ssutil.DbGetDiagStmt(hStmt, "createTTStatRep06: Could not create temporary table: " + y_cStatTab);
                ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                Ssutil.DisConn(hConn);
                return -6;
            }
            ...
            if (!ODBC.IsOK(sqlRet) && !ODBC.IsNoData(sqlRet))
            {
                Log2.e("\nTpReport.CreateTTStatRep(): ERROR: G. Could not update temporary table: " + cStatTab);
                Ssutil.DbGetDiagStmt(hStmt, "createTTStatRep07: Could not update temporary table: " + cStatTab);
                ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                Ssutil.DisConn(hConn);
                return -7;
            }
            ...
            if (!ODBC.IsOK(sqlRet) && !ODBC.IsNoData(sqlRet))
            {
                Log2.e("\nTpReport.CreateTTStatRep(): ERROR: H. Could not update temporary table: " + cStatTab);
                Ssutil.DbGetDiagStmt(hStmt, "createTTStatRep08: Could not update temporary table: " + cStatTab);
                ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                Ssutil.DisConn(hConn);
                return -8;
            }
            ...
            + in 1 or 2 places, the TSIP tables or the environment tables (mt_site).  We
            ...
            if (!ODBC.IsOK(sqlRet) && !ODBC.IsNoData(sqlRet))
            {
                Log2.e("\nTpReport.CreateTTStatRep(): ERROR: I. Could not update temporary table: " + cStatTab);
                Ssutil.DbGetDiagStmt(hStmt, "createTTStatRep09: Could not update temporary table: " + cStatTab);
                ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                Ssutil.DisConn(hConn);
                return -9;
            }
            ...
            if (!ODBC.IsOK(sqlRet) && !ODBC.IsNoData(sqlRet))
            {
                Log2.e("\nTpReport.CreateTTStatRep(): ERROR: J. Could not update temporary table: " + cStatTab);
                Ssutil.DbGetDiagStmt(hStmt, "createTTStatRep10: Could not update temporary table: " + cStatTab);
                ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                Ssutil.DisConn(hConn);
                return -10;
            }
                                         ...
                                         out string cEnvTab)
        {
            ...
            cSiteName = siteName;      /*  Copy the site name for ingres */
                                       ...
                                       +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
            GenUtil.MkUnique(out cPropTab, Info.GlobalSchema, "", "tsip_prop", "", "");
            GenUtil.MkUnique(out cEnvTab, Info.GlobalSchema, "", "tsip_env", "", "");

            string x_cPropTab, y_cPropTab, x_cEnvTab, y_cEnvTab;
            GenUtil.MkUnique(out x_cPropTab, Info.GlobalSchema, "x_", "tsip_prop", "", "");
            GenUtil.MkUnique(out y_cPropTab, Info.GlobalSchema, "y_", "tsip_prop", "", "");
            GenUtil.MkUnique(out x_cEnvTab, Info.GlobalSchema, "x_", "tsip_env", "", "");
            GenUtil.MkUnique(out y_cEnvTab, Info.GlobalSchema, "y_", "tsip_env", "", "");
