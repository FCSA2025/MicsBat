# Documented File: DynSdbTowr.cs
**Repository Path:** `_Utillib\DynSdbTowr.cs`
...
namespace _Utillib
{
    ...
    /// <b>main.sd_towr</b>.
    /// </summary>
    public class DynSdbTowr
    {
        private const string mBaseTableName = "sd_towr";
        ...
        private const string mUPDATE = "UPDATE {0} SET {1} WHERE twcode='{2}' ";

        private const string mINSERT = "INSERT INTO {0} VALUES ({1}) ";

        private const string mDELETE = "DELETE FROM {0} WHERE twcode='{1}' ";

        private const string mCHNG_UPDATE = "UPDATE {0} SET twcode='{1}', mdate='{2}', mtime='{3}' WHERE twcode='{4}' ";
        ...
        /// <summary>
        /// This method can be used to set whether the SDB <b>main.sd_towr</b> table should be written to,
        /// (SQL DELETE, INSERT and/or UPDATE queries) or not; it can also set whether the SQL queries should
        /// target the actual main.sd_towr table or a user's 'spoof' table (e.g. hulme.sd_towr).
        /// </summary>
        /// <param name="mdbWriteEnabled"> - boolean that prescribes whether sd_towr record
        ...
        public static void SetState(bool mdbWriteEnabled, bool spoofModeIsOff, string userSchema)
        {
            ...
            else
            {
                ...
                {
                    Log2.e("\nDynSdbTowr.SetState(): ERROR: spoofMode is ON but userSchema is null or whitespace.");
        ...
        private static int GetNextFreeCursor()
        {
            ...
            //...Log2.v("\r\nDynSdbTowr.GetNextFreeCursor(): nNextFreeCursor = " + nNextFreeCursor);
            if (nNextFreeCursor >= Constant.NUM_CURSORS_FEW)
            {
                ...
                {
                    //...Log2.v("\r\nDynSdbTowr.GetNextFreeCursor(): nInd = " + nInd);
                ...
                if (curHandle == -1)
                {
                    ...
                    GenUtil.SetErr("DynSdbTowr01 -- No more open cursors.");
                    Application.Exit("\r\nDynSdbTowr.GetNextFreeCursor(): ERROR: No available cursors.");
                    return -1;
                }
            ...
            //...Log2.v("\r\nDynSdbTowr.GetNextFreeCursor(): found available cursor: curHandle = " + curHandle);
        ...
        /// <summary>
        /// Selects records from the database table <b>main.sd_towr</b> i.a.w.
        ...
        /// <returns></returns>
        /// <para>- non-negative value - the index of the cursor to be used for SdFetchTowr() calls.</para>
        /// <para>- ErrorMessages.NO_CURSOR_AVAILABLE - reached limit for the number of cursors that can be open concurrently.</para>
        /// <para>-   ErrorMessages.ODBC_EXECUTE_FAILED    - call to ODBC.SQLExecute() failed to return data. </para>
        public static int SdSelectTowr(string searchCriteria, string orderBy)
        {
            //...Log2.v("\n\nDynSdbTowr.SdSelectTowr(): Entry");
            ...
            if (mhStmt == SQLHANDLE.Zero)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    Log2.e("\nDynSdbTowr.SdSelectTowr(): ERROR : Exit: call to SQLAllocHandle() failed, sqlRet = " + sqlRet);
                    return Error.ODBC_SQLALLOCHANDLE_FAILED;
                }

                SdTowr.BindPtrsToCols(mhStmt, out mTgtValPtrs, out mNullIndPtrs);
            }
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\nDynSdbTowr.SdSelectTowr(): ERROR : Exit: Attempt to close the cursor on the reusable statement handle failed.");
            ...
            //...Log2.v("\r\nDynSdbTowr.SdSelectTowr(): stmt_buf = \r\n" + stmt_buf);

            sqlRet = ODBC.SQLExecDirect(mhStmt, stmt_buf, stmt_buf.Length);

            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\nDynSdbTowr.SdSelectTowr(): ERROR : Exit: call to SQLExecDirect() failed, sqlRet = " + sqlRet);
            ...
            //...Log2.v("\n\nDynSdbTowr.SdSelectTowr(): Exit");
            return (curHandle);
        }

        /// <summary>
        /// Retrieves a single equipment record from the database table <b>main.sd_towr</b> using 
        /// the cursor object created by a previous call to SdSelectTowr().
        /// </summary>
        /// <param name="nCursor"> - FtCursor object.</param>
        /// <param name="sdTowr"> - a SdTowr object populated with data from the row.</param>
        /// <param name="nullInds"> - array of ODBC nullInds for sdTowr.</param>
        /// <returns></returns>
        ...
        public static int SdFetchTowr(int nCursor, out SdTowr sdTowr, out SQLLEN[] nullInds)
        {
            // 'out' requirments.
            sdTowr = null;
            nullInds = null;

            //...Log2.v("\n\nDynSdbTowr.SdFetchTowr(): Entry");

            FetchCount.IncrementSdTowr();
            ...
            {
                Log2.e("\n\nDynSdbTowr.SdFetchTowr(): Exit: ERROR: failed contiguity test.");

                return -666;
            }

            // Fetch the next row of selected Towr data.
            SQLRETURN sqlRet = ODBC.SQLFetch(mhStmt);

            // Check the SQLRETURN and act accordingly.
            if (!ODBC.IsOK(sqlRet))
            {
                // If no more Towr records available then break out the while-loop.
                if (sqlRet == ODBC.SQL_NO_DATA)
                {
                    //...Log2.v("\n\nDynSdbTowr.SdFetchTowr(): Exit: sqlRet == ODBC.SQL_NO_DATA");
                    return ODBC.SQL_NO_DATA;
                }
                else
                {
                    // We have an error.
                    Log2.e("\n\nDynSdbTowr.SdFetchTowr(): Exit: ERROR: SQLFetch(): sqlRet = " + sqlRet);
            ...
            else
            {
                ...
                try
                {
                    SdTowr.ReadColBindings(mTgtValPtrs, mNullIndPtrs, out sdTowr, out nullInds);
                ...
                catch (Exception e)
                {
                    ...
                    Log2.e("\n\nDynSdbTowr.SdFetchTowr(): Exit: ERROR: call to ReadColBindings() threw an exception:\n" + e.Message);
                    return -22;
                } //try-catch

            } //if-else

            //...Log2.v(sdTowr.ToStringWN(nullInds));

            //...Log2.v("\n\nDynSdbTowr.SdFetchTowr(): Exit");
            return nRet;
        } //end of SdFetchTowr

        /// <summary>
        /// Closes (releases) a cursor instantiated by a previous call to SdSelectTowr(); 
        ...
        /// <para>-   ErrorMessages.DYN_CUR_NOT_OPEN  - FtCursor field cursorOpen is set to false.</para>
        public static int SdCloseTowr(int curHandle)
        {
        ...
        /// <summary>
        /// This method updates all of the column values of a prescribed record in the SDB Towr table
        /// main.sd_towr.
        /// </summary>
        /// <param name="hConn"> - an open ODBC connection handle.</param>
        /// <param name="sdTowr"> - a prescribed SdTowr object that provides the updated field values.</param>
        /// <param name="nullInds"> - an array of ODBC nullInds associated with sdTowr.</param>
        /// <returns></returns>
        public static int SdUpdateTowr(SQLHANDLE hConn, SdTowr sdTowr, SQLLEN[] nullInds)
        {
            //...Log2.v("\nDynSdbTowr.SdUpdateTowr(): Entry");
            ...
            update_buf = String.Format(mUPDATE, mTableName, sdTowr.ToStringAsCSVequates(nullInds), sdTowr.twcode);

            //...Log2.v("\nDynSdbTowr.SdUpdateTowr(): query = " + update_buf);
            ...
            if (mMdbWriteEnabled)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    string str = String.Format("\nDynSdbTowr.SdUpdateTowr(): ERROR: SQLExecDirect failed, sqlRet = {0}, SQL query:\n{1}", sqlRet, update_buf);
                    Log2.e(str);
                    str = String.Format("mtUpdateTowr04 -- Error writing Towr: {0}", sdTowr.twcode);
            ...
            {
                Info.Text += "\r\n\r\n" + update_buf;
                //...Log2.v("\nDynSdbTowr.SdUpdateTowr(): Updating of main.sd_towr records is DISABLED.");
            }

            ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);

            //...Log2.v("\nDynSdbTowr.SdUpdateTowr(): Exit");
            return (Constant.SUCCESS);
        }

        /// <summary>
        /// This method inserts a record into the SDB Towr table main.sd_towr using column values prescribed 
        /// by the fields of a prescribed SdTowr object.
        /// </summary>
        /// <param name="hConn"> - an open ODBC connection handle.</param>
        /// <param name="sdTowr"> - a SdTowr object.</param>
        /// <param name="nullInd"> - array of ODBC nullInds for sdTowr</param>
        /// <returns></returns>
        ...
        public static int SdInsertTowr(SQLHANDLE hConn, SdTowr sdTowr, SQLLEN[] nullInd)
        {
            //...Log2.v("\n\nDynSdbTowr.SdInsertTowr(): Entry");
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\n\nDynSdbTowr.SdInsertTowr(): ERROR: call to SQLAllocHandle() failed.");
                return Error.ODBC_SQLALLOCHANDLE_FAILED;
            }

            //Create the SQL insert statement.
            cSQL = String.Format(mINSERT, mTableName, sdTowr.ToStringAsCSV(nullInd));

            //...Log2.v("nDynSdTowr.SdInsertTowr(): SQLExecDirect():\r\n" + cSQL);
            ...
            if (mMdbWriteEnabled)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    Log2.e("\n\nDynSdbTowr.SdInsertTowr(): ERROR: a call to SQLExecDirect() failed for: " + cSQL);
                    string str = "mtInsertTowr02 -- Error inserting antenna";
                    Ssutil.DbGetDiagStmt(hStmt, str); ;
                    ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                    return Error.ODBC_EXECDIRECT_FAILED;
                }

                //...Log2.v("\n\nDynSdbTowr.SdInsertTowr(): successfuly inserted record into table main.sd_towr");
            }
            else
            {
                Info.Text += "\r\n\r\n" + cSQL;
                //...Log2.v("\nDynSdbTowr.SdInsertTowr(): Insertion of main.sd_towr records is DISABLED.");
            }

            //This method call releases hStmt, disconnects from the DB and then releases hConn.
            ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);

            //...Log2.v("\n\nDynSdbTowr.SdInsertTowr(): Exit");
            return Constant.SUCCESS;
        }

        /// <summary>
        /// This method deletes a record with a prescribed twcode from the SDB
        /// Towr table main.sd_towr.
        /// </summary>
        /// <param name="hConn"> - an open ODBC connection handle.</param>
        /// <param name="twcode"> - a prescribed antenna code.</param>
        /// <returns></returns>
        public static int SdDeleteTowr(SQLHANDLE hConn, string twcode)
        {
            //...Log2.v("\n\nDynSdbTowr.SdDeleteTowr(): Entry");
            ...
            string delete_buf = String.Format(mDELETE, mTableName, twcode);

            //...Log2.v("\nDynSdbTowr.SdDeleteTowr(): query = " + delete_buf);
            ...
            if (mMdbWriteEnabled)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    string str = String.Format("mtDeleteTowr01 -- Error deleting from main.sd_towr");
                    Ssutil.DbGetDiagStmt(hStmt, str);
                    ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                    Log2.e("\nDynSdbTowr.SdDeleteTowr(): ERROR: SQLExecDirect() failed for query = " + delete_buf);
                    return Error.ODBC_EXECDIRECT_FAILED;
                }
            }
            else
            {
                Info.Text += "\r\n\r\n" + delete_buf;
                //...Log2.v("\nDynSdbTowr.SdDeleteTowr(): Deletion of main.sd_towr records is DISABLED.");
            }

            ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);

            //...Log2.v("\n\nDynSdbTowr.SdDeleteTowr(): Exit");
            return 0;
        }

        /// <summary>
        /// This method returns true if a record exists in the SDB Towr table <b>main.sd_towr</b>
        /// that has a prescribed tower code (twcode).
        /// </summary>
        /// <param name="twcode"> the prescribed 'call sign' string.</param>
        /// <returns></returns>
        public static bool SdbRecordExistsWithEcode(string twcode)
        {
            string whereClause = String.Format(" twcode = '{0}' ", twcode);
        ...
        /// <summary>
        /// This method returns a populated SdTowr object and associated ODBC nullInds
        /// corresponding to the record in the main SDB Towr table 'main.sd_towr' that has the 
        /// prescribed key (twcode).
        /// </summary>
        /// <param name="twcode"></param>
        ...
        public static bool FetchRecordWithEcode(string twcode, out SdTowr sdTowr, out SQLLEN[] nullInds)
        {
            // 'out' requirements.
            sdTowr = null;
            nullInds = null;
            ...
            // Do the 'SELECT'.
            int nCursor = SdSelectTowr(whereClause, "");

            if (nCursor < 0)
            {
                Log2.e("\nDynSdbTowr.FetchRecordWithKeys(): ERROR: Select() failed, nCursor = " + nCursor);
                return false;
            }

            // Do the 'FETCH'.
            int rc = SdFetchTowr(nCursor, out sdTowr, out nullInds);
            ...
            {
                Log2.e("\nDynSdbTowr.FetchRecordWithKeys(): ERROR: Fetch() failed, rc = " + rc);
                isSuccessful = false;
            }
