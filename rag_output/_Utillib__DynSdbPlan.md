# Documented File: DynSdbPlan.cs
**Repository Path:** `_Utillib\DynSdbPlan.cs`
...
namespace _Utillib
{
    ...
    /// <summary>
    /// This class provides methods to select and fetch PLAN records from the subsiduary database table
    /// <b>main.sd_plan</b>.
    /// </summary>
    public class DynSdbPlan
    {
        private const string mBaseTableName = "sd_plan";
        ...
        private const string mUPDATE = "UPDATE {0} SET {1} WHERE sband='{2}' AND splan='{3}' ";

        private const string mINSERT = "INSERT INTO {0} VALUES ({1}) ";

        private const string mDELETE = "DELETE FROM {0} WHERE sband='{1}' AND splan='{2}' ";
        ...
        /// <summary>
        /// This method can be used to set whether the SDB <b>main.sd_plan</b> table should be written to,
        /// (SQL DELETE, INSERT and/or UPDATE queries) or not; it can also set whether the SQL queries should
        /// target the actual main.sd_plan table or a user's 'spoof' table (e.g. hulme.sd_plan).
        /// </summary>
        /// <param name="mdbWriteEnabled"> - boolean that prescribes whether sd_plan record
        ...
        public static void SetState(bool mdbWriteEnabled, bool spoofModeIsOff, string userSchema)
        {
            ...
            else
            {
                ...
                {
                    Log2.e("\nDynSdbPlan.SetState(): ERROR: spoofMode is ON but userSchema is null or whitespace.");
        ...
        private static int GetNextFreeCursor()
        {
            ...
            //...Log2.v("\r\nDynSdbPlan.GetNextFreeCursor(): nNextFreeCursor = " + nNextFreeCursor);
            if (nNextFreeCursor >= Constant.NUM_CURSORS_FEW)
            {
                ...
                {
                    //...Log2.v("\r\nDynSdbPlan.GetNextFreeCursor(): nInd = " + nInd);
                ...
                if (curHandle == -1)
                {
                    ...
                    GenUtil.SetErr("DynSdbPlan01 -- No more open cursors.");
                    Application.Exit("\r\nDynSdbPlan.GetNextFreeCursor(): ERROR: No available cursors.");
                    return -1;
                }
            ...
            //...Log2.v("\r\nDynSdbPlan.GetNextFreeCursor(): found available cursor: curHandle = " + curHandle);
        ...
        /// <summary>
        /// Selects PLAN records from the database table <b>main.sd_plan</b> for
        ...
        /// <returns></returns>
        /// <para>- non-negative value - the index of the cursor to be used for SdFetchPlan() calls.</para>
        /// <para>- ErrorMessages.NO_CURSOR_AVAILABLE - reached limit for the number of cursors that can be open concurrently.</para>
        /// <para>-   ErrorMessages.ODBC_EXECUTE_FAILED    - call to ODBC.SQLExecute() failed to return data. </para>
        public static int SdSelectPlan(string searchCriteria, string orderBy)
        {
            //...Log2.v("\n\nDynSdbPlan.SdSelectPlan(): Entry");
            ...
            if (mhStmt == SQLHANDLE.Zero)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    Log2.e("\nDynSdbPlan.SdSelectPlan(): ERROR : Exit: call to SQLAllocHandle() failed, sqlRet = " + sqlRet);
                    return Error.ODBC_SQLALLOCHANDLE_FAILED;
                }

                SdPlan.BindPtrsToCols(mhStmt, out mTgtValPtrs, out mNullIndPtrs);
            }
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\nDynSdbPlan.SdSelectPlan(): ERROR : Exit: Attempt to close the cursor on the reusable statement handle failed.");
            ...
            //...Log2.v("\r\nDynSdbPlan.SdSelectPlan(): stmt_buf = \r\n" + stmt_buf);

            sqlRet = ODBC.SQLExecDirect(mhStmt, stmt_buf, stmt_buf.Length);

            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\nDynSdbPlan.SdSelectPlan(): ERROR : Exit: call to SQLExecDirect() failed, sqlRet = " + sqlRet);
            ...
            //...Log2.v("\n\nDynSdbPlan.SdSelectPlan(): Exit");
            return (curHandle);
        }

        /// <summary>
        /// Retrieves a single row of PLAN data from the database table <b>main.sd_plan</b> using 
        /// the cursor object created by a previous call to SdSelectPlan().
        /// </summary>
        /// <param name="nCursor"> - FtCursor object.</param>
        /// <param name="sdPlan"> - a SdPlan object populated with data from the row.</param>
        /// <param name="nullInds"> - array of ODBC nullInds for sdPlan.</param>
        /// <returns></returns>
        ...
        public static int SdFetchPlan(int nCursor, out SdPlan sdPlan, out SQLLEN[] nullInds)
        {
            // 'out' requirments.
            sdPlan = null;
            nullInds = null;

            //...Log2.v("\n\nDynSdbPlan.SdFetchPlan(): Entry");
            ...
            {
                Log2.e("\n\nDynSdbPlan.SdFetchPlan(): Exit: ERROR: failed contiguity test.");

                return -666;
            }

            // Fetch the next row of selected Plan data.
            SQLRETURN sqlRet = ODBC.SQLFetch(mhStmt);

            // Check the SQLRETURN and act accordingly.
            if (!ODBC.IsOK(sqlRet))
            {
                // If no more Plan records available then break out the while-loop.
                if (sqlRet == ODBC.SQL_NO_DATA)
                {
                    //...Log2.v("\n\nDynSdbPlan.SdFetchPlan(): Exit: sqlRet == ODBC.SQL_NO_DATA");
                ...
                {
                    // We have an error.
                    Log2.e("\n\nDynSdbPlan.SdFetchPlan(): Exit: ERROR: SQLFetch(): sqlRet = " + sqlRet);
            ...
            else
            {
                ...
                try
                {
                    SdPlan.ReadColBindings(mTgtValPtrs, mNullIndPtrs, out sdPlan, out nullInds);
                ...
                catch (Exception e)
                {
                    ...
                    Log2.e("\n\nDynSdbPlan.SdFetchPlan(): Exit: ERROR: call to ReadColBindings() threw an exception:\n" + e.Message);
                    return -22;
                } //try-catch

            } //if-else

            //...Log2.v(sdPlan.ToStringWN(nullInds));

            //...Log2.v("\n\nDynSdbPlan.SdFetchPlan(): Exit");
            return nRet;
        } //end of SdFetchPlan

        /// <summary>
        /// Closes (releases) a cursor associated with a PLAN record; 
        ...
        /// <para>-   ErrorMessages.DYN_CUR_NOT_OPEN  - FtCursor field cursorOpen is set to false.</para>
        public static int SdClosePlan(int curHandle)
        {
        ...
        /// <summary>
        /// This method updates all of the column values of a prescribed record in the SDB PLAN table
        /// main.sd_plan.
        /// </summary>
        /// <param name="hConn"> - an open ODBC connection handle.</param>
        /// <param name="sdPlan"> - a prescribed SdPlan object that provides the updated field values.</param>
        /// <param name="nullInds"> - an array of ODBC nullInds associated with sdPlan.</param>
        /// <returns></returns>
        public static int SdUpdatePlan(SQLHANDLE hConn, SdPlan sdPlan, SQLLEN[] nullInds)
        {
            //...Log2.v("\nDynSdbPlan.SdUpdatePlan(): Entry");
            ...
            update_buf = String.Format(mUPDATE, mTableName, sdPlan.ToStringAsCSVequates(nullInds), sdPlan.sband, sdPlan.splan);

            //...Log2.v("\nDynSdbPlan.SdUpdatePlan(): query = " + update_buf);
            ...
            if (mMdbWriteEnabled)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    string str = String.Format("\nDynSdbPlan.SdUpdatePlan(): ERROR: SQLExecDirect failed, sqlRet = {0}, SQL query:\n{1}", sqlRet, update_buf);
                    Log2.e(str);
                    str = String.Format("mtUpdatePlan04 -- Error writing Plan: {0}", sdPlan);
            ...
            {
                Info.Text += "\r\n\r\n" + update_buf;
                //...Log2.v("\nDynSdbPlan.SdUpdatePlan(): Updating of main.sd_plan records is DISABLED.");
            }

            ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);

            //...Log2.v("\nDynSdbPlan.SdUpdatePlan(): Exit");
            return (Constant.SUCCESS);
        }

        /// <summary>
        /// Inserts a record into the SDB table main.sd_plan using column values prescribed 
        /// by the fields of an SdPlan object.
        /// </summary>
        /// <param name="hConn"> - an open ODBC connection handle.</param>
        /// <param name="sdPlan"> - a SdPlan object.</param>
        /// <param name="nullInd"> - array of ODBC nullInds for sdPlan</param>
        /// <returns></returns>
        ...
        public static int SdInsertPlan(SQLHANDLE hConn, SdPlan sdPlan, SQLLEN[] nullInd)
        {
            //...Log2.v("\n\nDynSdbPlan.SdInsertPlan(): Entry");
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\n\nDynSdbPlan.SdInsertPlan(): ERROR: call to SQLAllocHandle() failed.");
                return Error.ODBC_SQLALLOCHANDLE_FAILED;
            }

            //Create the SQL insert statement.
            cSQL = String.Format(mINSERT, mTableName, sdPlan.ToStringAsCSV(nullInd));

            //...Log2.v("nDynSdPlan.SdInsertPlan(): SQLExecDirect():\r\n" + cSQL);
            ...
            if (mMdbWriteEnabled)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    Log2.e("\n\nDynSdbPlan.SdInsertPlan(): ERROR: a call to SQLExecDirect() failed for: " + cSQL);
                    string str = "mtInsertPlan02 -- Error inserting PLAN";
                    Ssutil.DbGetDiagStmt(hStmt, str); ;
                    ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                    return Error.ODBC_EXECDIRECT_FAILED;
                }

                //...Log2.v("\n\nDynSdbPlan.SdInsertPlan(): successfuly inserted record into table main.sd_plan");
            }
            else
            {
                Info.Text += "\r\n\r\n" + cSQL;
                //...Log2.v("\nDynSdbPlan.SdInsertPlan(): Insertion of main.sd_plan records is DISABLED.");
            }

            //This method call releases hStmt, disconnects from the DB and then releases hConn.
            ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);

            //...Log2.v("\n\nDynSdbPlan.SdInsertPlan(): Exit");
            return Constant.SUCCESS;
        }
        ...
        /// PLAN table main.sd_plan.
        /// </summary>
        /// <param name="hConn"></param>
        /// <param name="sband"></param>
        /// <param name="splan"></param>
        /// <returns></returns>
        public static int SdDeletePlan(SQLHANDLE hConn, string sband, string splan)
        {
            //...Log2.v("\n\nDynSdbPlan.SdDeletePlan(): Entry");
            ...
            string delete_buf = String.Format(mDELETE, mTableName, sband, splan);

            //...Log2.v("\nDynSdbPlan.SdDeletePlan(): query = " + delete_buf);
            ...
            if (mMdbWriteEnabled)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    string str = String.Format("mtDeletePlan01 -- Error deleting from main.sd_plan");
                    Ssutil.DbGetDiagStmt(hStmt, str);
                    ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                    Log2.e("\nDynSdbPlan.SdDeletePlan(): ERROR: SQLExecDirect() failed for query = " + delete_buf);
                    return Error.ODBC_EXECDIRECT_FAILED;
                }
            }
            else
            {
                Info.Text += "\r\n\r\n" + delete_buf;
                //...Log2.v("\nDynSdbPlan.SdDeletePlan(): Deletion of main.sd_plan records is DISABLED.");
            }

            ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);

            //...Log2.v("\n\nDynSdbPlan.SdDeletePlan(): Exit");
            return 0;
        }

        /// <summary>
        /// This method returns true if a record exists in the SDB table <b>main.sd_plan</b>
        /// that has a prescribed PLAN key {sband, splan}.
        /// </summary>
        /// <param name="sband"></param>
        /// <param name="splan"></param>
        /// <returns></returns>
        public static bool SdbRecordExistsWithKeys(string sband, string splan)
        {
            string whereClause = String.Format(" sband = '{0}' AND splan = '{1}' ", sband, splan);
