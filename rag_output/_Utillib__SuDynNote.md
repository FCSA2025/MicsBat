# Documented File: SuDynNote.cs
**Repository Path:** `_Utillib\SuDynNote.cs`
...
namespace _Utillib
{
    ...
    /// <summary>
    /// This class provides methods that serve SuNote objects to/from a database.
    /// </summary>
    public class SuDynNote
    {
        //===================================================================================================================================
        ...
        public static int GetNextFreeCursor()
        {
            ...
            if (nNextFreeCursor >= Constant.NUM_CURSORS_FEW)
            {
                ...
                if (curHandle == -1)
                {
                    Application.Exit("\r\nSuDynNote.GetNextFreeCursor(): ERROR: No available cursors.");
                }
            }
            else
            {
        ...
        /// <summary>
        /// Selects SuNote records from a user's su_XXX_note DB table for
        ...
        /// <returns></returns>
        /// <para>- non-negative value - the index of the cursor to be used for SuFetchNote() calls.</para>
        /// <para>- ErrorMessages.NO_CURSOR_AVAILABLE - reached limit for the number of cursors that can be open concurrently.</para>
        /// <para>-   ErrorMessages.ODBC_EXECDIRECT_FAILED    - call to ODBC.SQLExecute() failed to return data. </para>
        public static int SuSelectNote(string table, string searchCriteria, string orderBy)
        {
            //...Log2.v("\n\nSuDynNote.SuSelectNote(): Entry");
            SQLHANDLE hStmt;
            SQLHANDLE hUpdate;
            SQLRETURN sqlRet;
            ...
            string stmt_buf = "SELECT " + SuNote.AllColumnsForSqlSelect + " FROM " + table;
            ...
            //...Log2.v("\r\nSuDynNote.SuSelectNote(): [1] sqlRet = " + sqlRet);

            //...Log2.v("\nSuDynNote.SuSelectNote(): query = " + stmt_buf);

            sqlRet = ODBC.SQLExecDirect(hStmt, stmt_buf, ODBC.SQL_NTS);

            //...Log2.v("\r\nSuDynNote.SuSelectNote(): [4] sqlRet = " + sqlRet);

            if (!ODBC.IsOK(sqlRet))
            {
                Ssutil.DbGetDiagStmt(hStmt, "Could not Select Note for criteria: " + searchCriteria + ".");
                ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                Ssutil.DisConn(hConn);
                Log2.e("\nSuDynNote.SuSelectNote(): ERROR: SQLExecute() failed");
            ...
            //...Log2.v("\n\nSuDynNote.SuSelectNote(): Exit");
            return curHandle;
        }

        /// <summary>
        /// Retrieves a single row of SuNote data from a table in the database using 
        /// a Cursor object previously created by a call to SuSelectNote().
        /// </summary>
        /// <param name="curHandle"> - index of a Cursor object.</param>
        /// <param name="suNote"> - a SuNote object populated with data from the row.</param>
        /// <param name="nullInd"> - array of ODBC nullInds for suNote.</param>
        /// <returns></returns>
        ...
        public static short SuFetchNote(int curHandle, out SuNote suNote, out SQLLEN[] nullInd)
        {
            //...Log2.v("\n\nSuDynNote.SuFetchNote(): Entry:");

            //Create an SuNote object to output.
            suNote = new SuNote();
            //Create an array of nulls to output.
            nullInd = new SQLLEN[SuNote.NUM_COLUMNS];
            ...
            if (!cursors[curHandle].cursorOpen)
            {
                ...
                Log2.e("\n\nSuDynNote.SuFetchNote(): Exit: FAIL: if (!cursors[curHandle].cursorOpen)");
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                ...
                {
                    //...Log2.v("\n\nSuDynNote.SuFetchNote(): Exit: ODBC.SQLFetch(): sqlRet == ODBC.SQL_NO_DATA");
                    return sqlRet;
                }
                else
                {
                    //	Error
                    Log2.e("\n\nSuDynNote.SuFetchNote(): Exit: ODBC.SQLFetch(): FAIL: sqlRet = " + sqlRet);
            ...
            try
            {
                Ssutil.DbGetString(hStmt, 1, "cmd", out suNote.cmd, SuNote.CMD_SZ, out nullInd[SuNote.CMD]);
                Ssutil.DbGetString(hStmt, 2, "recstat", out suNote.recstat, SuNote.RECSTAT_SZ, out nullInd[SuNote.RECSTAT]);
                Ssutil.DbGetString(hStmt, 3, "oper", out suNote.oper, SuNote.OPER_SZ, out nullInd[SuNote.OPER]);
                Ssutil.DbGetString(hStmt, 4, "nonum", out suNote.nonum, SuNote.NONUM_SZ, out nullInd[SuNote.NONUM]);
                Ssutil.DbGetString(hStmt, 5, "note", out suNote.note, SuNote.NOTE_SZ, out nullInd[SuNote.NOTE]);
                Ssutil.DbGetString(hStmt, 6, "mdate", out suNote.mdate, SuNote.MDATE_SZ, out nullInd[SuNote.MDATE]);
                Ssutil.DbGetString(hStmt, 7, "mtime", out suNote.mtime, SuNote.MTIME_SZ, out nullInd[SuNote.MTIME]);
            }
            catch (Exception e)
            {
                Log2.e("\n\nSuDynNote.SuFetchNote(): ERROR: Exception caught: " + e.Message);
                GenUtil.SetErr("dynfeNote02 -- Could not get note, because of field " + e.Message);
                return Error.ODBC_GET_FAILED;
            }

            //...Log2.v("\n\nSuDynNote.SuFetchNote(): Exit");
        ...
        /// <summary>
        /// Closes (releases) a cursor instantiated by a previous call to SuSelectNote(); 
        ...
        public static int SuCloseNote(int curHandle)
        {
