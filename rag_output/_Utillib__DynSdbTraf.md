# Documented File: DynSdbTraf.cs
**Repository Path:** `_Utillib\DynSdbTraf.cs`
...
namespace _Utillib
{
    ...
    /// <b>main.sd_traf</b>.
    /// </summary>
    public class DynSdbTraf
    {
        private const string mBaseTableName = "sd_traf";
        ...
        private const string mUPDATE = "UPDATE {0} SET {1} WHERE trafcode='{2}' AND ecode='{3}' ";

        private const string mINSERT = "INSERT INTO {0} VALUES ({1}) ";

        private const string mDELETE = "DELETE FROM {0} WHERE trafcode='{1}' AND ecode='{2}' ";

        private const string mCHNG_UPDATE = "UPDATE {0} SET trafcode='{1}', mdate='{2}', mtime='{3}' WHERE trafcode='{4}' ";
        ...
        /// <summary>
        /// This method can be used to set whether the SDB <b>main.sd_traf</b> table should be written to,
        /// (SQL DELETE, INSERT and/or UPDATE queries) or not; it can also set whether the SQL queries should
        /// target the actual main.sd_traf table or a user's 'spoof' table (e.g. hulme.sd_traf).
        /// </summary>
        /// <param name="mdbWriteEnabled"> - boolean that prescribes whether sd_traf record
        /// DELETE, INSERT and/or UPDATE trafcodeations will be performed (the default) or inhibited.</param>
        ...
        private static int GetNextFreeCursor()
        {
            ...
            //...Log2.v("\r\nDynSdbTraf.GetNextFreeCursor(): nNextFreeCursor = " + nNextFreeCursor);
            if (nNextFreeCursor >= Constant.NUM_CURSORS_FEW)
            {
                ...
                {
                    //...Log2.v("\r\nDynSdbTraf.GetNextFreeCursor(): nInd = " + nInd);
                ...
                if (curHandle == -1)
                {
                    ...
                    GenUtil.SetErr("DynSdbTraf01 -- No more open cursors.");
                    Application.Exit("\r\nDynSdbTraf.GetNextFreeCursor(): ERROR: No available cursors.");
                    return -1;
                }
            ...
            //...Log2.v("\r\nDynSdbTraf.GetNextFreeCursor(): found available cursor: curHandle = " + curHandle);
        ...
        /// <summary>
        /// Selects records from the database table <b>main.sd_traf</b> i.a.w.
        ...
        /// <returns></returns>
        /// <para>- non-negative value - the index of the cursor to be used for SdFetchTraf() calls.</para>
        /// <para>- ErrorMessages.NO_CURSOR_AVAILABLE - reached limit for the number of cursors that can be open concurrently.</para>
        /// <para>-   ErrorMessages.ODBC_EXECUTE_FAILED    - call to ODBC.SQLExecute() failed to return data. </para>
        public static int SdSelectTraf(string searchCriteria, string orderBy)
        {
            //...Log2.v("\n\nDynSdbTraf.SdSelectTraf(): Entry");
            ...
            if (mhStmt == SQLHANDLE.Zero)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    Log2.e("\nDynSdbTraf.SdSelectTraf(): ERROR : Exit: call to SQLAllocHandle() failed, sqlRet = " + sqlRet);
                    return Error.ODBC_SQLALLOCHANDLE_FAILED;
                }

                SdTraf.BindPtrsToCols(mhStmt, out mTgtValPtrs, out mNullIndPtrs);
            }
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\nDynSdbTraf.SdSelectTraf(): ERROR : Exit: Attempt to close the cursor on the reusable statement handle failed.");
            ...
            //...Log2.v("\r\nDynSdbTraf.SdSelectTraf(): stmt_buf = \r\n" + stmt_buf);

            sqlRet = ODBC.SQLExecDirect(mhStmt, stmt_buf, stmt_buf.Length);

            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\nDynSdbTraf.SdSelectTraf(): ERROR : Exit: call to SQLExecDirect() failed, sqlRet = " + sqlRet);
            ...
            //...Log2.v("\n\nDynSdbTraf.SdSelectTraf(): Exit");
            return (curHandle);
        }

        /// <summary>
        /// Retrieves a single equipment record from the database table <b>main.sd_traf</b> using 
        /// the cursor object created by a previous call to SdSelectTraf().
        /// </summary>
        /// <param name="nCursor"> - FtCursor object.</param>
        /// <param name="sdTraf"> - a SdTraf object populated with data from the row.</param>
        /// <param name="nullInds"> - array of ODBC nullInds for sdTraf.</param>
        /// <returns></returns>
        ...
        public static int SdFetchTraf(int nCursor, out SdTraf sdTraf, out SQLLEN[] nullInds)
        {
            // 'out' requirments.
            sdTraf = null;
            nullInds = null;

            //...Log2.v("\n\nDynSdbTraf.SdFetchTraf(): Entry");
            ...
            {
                Log2.e("\n\nDynSdbTraf.SdFetchTraf(): Exit: ERROR: failed contiguity test.");

                return -666;
            }

            // Fetch the next row of selected Traf data.
            SQLRETURN sqlRet = ODBC.SQLFetch(mhStmt);

            // Check the SQLRETURN and act accordingly.
            if (!ODBC.IsOK(sqlRet))
            {
                // If no more Traf records available then break out the while-loop.
                if (sqlRet == ODBC.SQL_NO_DATA)
                {
                    //...Log2.v("\n\nDynSdbTraf.SdFetchTraf(): Exit: sqlRet == ODBC.SQL_NO_DATA");
                    return ODBC.SQL_NO_DATA;
                }
                else
                {
                    // We have an error.
                    Log2.e("\n\nDynSdbTraf.SdFetchTraf(): Exit: ERROR: SQLFetch(): sqlRet = " + sqlRet);
            ...
            {
                // We had a successful SQLFetch() trafcodeation; now get the record's column data.
                try
                {
                    SdTraf.ReadColBindings(mTgtValPtrs, mNullIndPtrs, out sdTraf, out nullInds);
                ...
                catch (Exception e)
                {
                    ...
                    Log2.e("\n\nDynSdbTraf.SdFetchTraf(): Exit: ERROR: call to ReadColBindings() threw an exception:\n" + e.Message);
                    return -22;
                } //try-catch

            } //if-else

            //...Log2.v(sdTraf.ToStringWN(nullInds));

            //...Log2.v("\n\nDynSdbTraf.SdFetchTraf(): Exit");
            return nRet;
        } //end of SdFetchTraf

        /// <summary>
        /// Closes (releases) a cursor instantiated by a previous call to SdSelectTraf(); 
        ...
        /// <para>-   ErrorMessages.DYN_CUR_NOT_OPEN  - FtCursor field cursorOpen is set to false.</para>
        public static int SdCloseTraf(int curHandle)
        {
        ...
        /// <summary>
        /// This method updates all of the column values of a prescribed record in the SDB Traf table
        /// main.sd_traf.
        /// </summary>
        /// <param name="hConn"> - an open ODBC connection handle.</param>
        /// <param name="sdTraf"> - a prescribed SdTraf object that provides the updated field values.</param>
        /// <param name="nullInds"> - an array of ODBC nullInds associated with sdTraf.</param>
        /// <returns></returns>
        public static int SdUpdateTraf(SQLHANDLE hConn, SdTraf sdTraf, SQLLEN[] nullInds)
        {
            //...Log2.v("\nDynSdbTraf.SdUpdateTraf(): Entry");
            ...
            update_buf = String.Format(mUPDATE, mTableName, sdTraf.ToStringAsCSVequates(nullInds), sdTraf.trafcode, sdTraf.ecode);

            //...Log2.v("\nDynSdbTraf.SdUpdateTraf(): query = " + update_buf);
            ...
            if (mMdbWriteEnabled)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    string str = String.Format("\nDynSdbTraf.SdUpdateTraf(): ERROR: SQLExecDirect failed, sqlRet = {0}, SQL query:\n{1}", sqlRet, update_buf);
                    Log2.e(str);
                    str = String.Format("mtUpdateTraf04 -- Error writing Traf: {0} {1}", sdTraf.trafcode, sdTraf.ecode);
            ...
            {
                Info.Text += "\r\n\r\n" + update_buf;
                //...Log2.v("\nDynSdbTraf.SdUpdateTraf(): Updating of main.sd_traf records is DISABLED.");
            }

            ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);

            //...Log2.v("\nDynSdbTraf.SdUpdateTraf(): Exit");
            return (Constant.SUCCESS);
        }

        /// <summary>
        /// This method inserts a record into the SDB Traf table main.sd_traf using column values prescribed 
        /// by the fields of a prescribed SdTraf object.
        /// </summary>
        /// <param name="hConn"> - an open ODBC connection handle.</param>
        /// <param name="sdTraf"> - a SdTraf object.</param>
        /// <param name="nullInd"> - array of ODBC nullInds for sdTraf</param>
        /// <returns></returns>
        ...
        public static int SdInsertTraf(SQLHANDLE hConn, SdTraf sdTraf, SQLLEN[] nullInd)
        {
            //...Log2.v("\n\nDynSdbTraf.SdInsertTraf(): Entry");
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\n\nDynSdbTraf.SdInsertTraf(): ERROR: call to SQLAllocHandle() failed.");
                return Error.ODBC_SQLALLOCHANDLE_FAILED;
            }

            //Create the SQL insert statement.
            cSQL = String.Format(mINSERT, mTableName, sdTraf.ToStringAsCSV(nullInd));

            //...Log2.v("nDynSdTraf.SdInsertTraf(): SQLExecDirect():\r\n" + cSQL);
            ...
            if (mMdbWriteEnabled)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    Log2.e("\n\nDynSdbTraf.SdInsertTraf(): ERROR: a call to SQLExecDirect() failed for: " + cSQL);
                    string str = "mtInsertTraf02 -- Error inserting antenna";
                    Ssutil.DbGetDiagStmt(hStmt, str); ;
                    ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                    return Error.ODBC_EXECDIRECT_FAILED;
                }

                //...Log2.v("\n\nDynSdbTraf.SdInsertTraf(): successfuly inserted record into table main.sd_traf");
            }
            else
            {
                Info.Text += "\r\n\r\n" + cSQL;
                //...Log2.v("\nDynSdbTraf.SdInsertTraf(): Insertion of main.sd_traf records is DISABLED.");
            }

            //This method call releases hStmt, disconnects from the DB and then releases hConn.
            ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);

            //...Log2.v("\n\nDynSdbTraf.SdInsertTraf(): Exit");
            return Constant.SUCCESS;
        }

        /// <summary>
        /// This method deletes a record with prescribed keys (trafcode, ecode) from the SDB
        /// Traf table main.sd_traf.
        /// </summary>
        /// <param name="hConn"> - an open ODBC connection handle.</param>
        /// <param name="trafcode"></param>
        /// <param name="ecode"></param>
        /// <returns></returns>
        public static int SdDeleteTraf(SQLHANDLE hConn, string trafcode, string ecode)
        {
            //...Log2.v("\n\nDynSdbTraf.SdDeleteTraf(): Entry");
            ...
            string delete_buf = String.Format(mDELETE, mTableName, trafcode, ecode);

            //...Log2.v("\nDynSdbTraf.SdDeleteTraf(): query = " + delete_buf);
            ...
            if (mMdbWriteEnabled)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    string str = String.Format("mtDeleteTraf01 -- Error deleting from main.sd_traf");
                    Ssutil.DbGetDiagStmt(hStmt, str);
                    ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                    Log2.e("\nDynSdbTraf.SdDeleteTraf(): ERROR: SQLExecDirect() failed for query = " + delete_buf);
                    return Error.ODBC_EXECDIRECT_FAILED;
                }
            }
            else
            {
                Info.Text += "\r\n\r\n" + delete_buf;
                //...Log2.v("\nDynSdbTraf.SdDeleteTraf(): Deletion of main.sd_traf records is DISABLED.");
            }

            ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);

            //...Log2.v("\n\nDynSdbTraf.SdDeleteTraf(): Exit");
            return 0;
        }
