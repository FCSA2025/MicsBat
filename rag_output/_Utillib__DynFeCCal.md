# Documented File: DynFeCCal.cs
**Repository Path:** `_Utillib\DynFeCCal.cs`
...
namespace _Utillib
{
    ...
    public class DynFeCCal
    {
        ...
        public static int GetNextFreeCursor()
        {
            ...
            if (nNextFreeCursor >= Constant.NUM_CURSORS_FEW)
            {
                ...
                if (curHandle == -1)
                {
                    ...
                    Application.Exit("\r\nDynFeCCal.GetNextFreeCursor(): ERROR: No available cursors.");
                }
            }
            else
            {
        ...
        /// <summary>
        /// Selects site records from a '_CCal' table in the database for
        /// the prescribed site table name, SQL search criteria and ordering clauses; the 
        /// method returns an index to a FeCCal object that can be used by subsequent
        /// calls to FeFetchCCal().
        /// </summary>
        /// <remarks>
        ...
        public static int FeSelectCCal(string table, string searchCriteria, string orderBy)
        {
            //...Log2.v("\n\nDynFeCCal.FeSelectCCal(): Entry");
            SQLHANDLE hStmt;
            SQLRETURN sqlRet;
            int curHandle;          /* cursor handle */

            //Construct the select clause.
            string cSQL = "select " + FeCCal.AllColumnsForSqlSelect + " from " + table;
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("DynFeCCal.FeSelectCCal(): ERROR: call to SQLExecDirect() failed for: " + cSQL);
            ...
            //...Log2.v("\n\nDynFeCCal.FeSelectCCal(): Exit");
            return curHandle;
        }

        /// <summary>
        /// Closes (releases) a cursor associated with a PDF ES 'ccal' record; 
        ...
        public static int FeCloseCCal(int curHandle)
        {
        ...
        /// <summary>
        /// Retrieves a single row of FeCCal data from a table in the database using 
        /// a previously created Cursor object.
        /// </summary>
        /// <param name="curHandle"> - index of a Cursor object.</param>
        /// <param name="cCalRec"> - a FeCCal object populated with data from the row.</param>
        /// <param name="nullInd"> - array of ODBC nullInds for cCalRec.</param>
        /// <returns></returns>
        ...
        public static int FeFetchCCal(int curHandle, out FeCCal cCalRec, out SQLLEN[] nullInd)
        {
            //...Log2.v("\n\nDynFeCCal.FeFetchCCal(): Entry:");

            //Create an FeCCal object to output.
            cCalRec = new FeCCal();
            //Create an array of nulls to output.
            nullInd = new SQLLEN[FeCCal.NUM_COLUMNS];
            ...
            {
                /* Cursor isn't opened yet */
                Log2.e("\n\nDynFeCCal.FeFetchCCal(): Exit: FAIL: if (!cursors[curHandle].cursorOpen)");
                return (Error.DYN_CUR_NOT_OPEN);
            }
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                ...
                {
                    //...Log2.v("\n\nDynFeCCal.FeFetchCCal(): Exit: ODBC.SQLFetch(): sqlRet == ODBC.SQL_NO_DATA");
                    return (int)sqlRet;
                }
                else
                {
                    //	Error
                    Log2.e("\n\nDynFeCCal.FeFetchCCal(): Exit: ODBC.SQLFetch(): FAIL: sqlRet = " + sqlRet);
            ...
            try
            {
                Ssutil.DbGetString(hStmt, 1, "newcallsign", out cCalRec.newcallsign, FeCCal.NEWCALLSIGN_SZ, out nullInd[FeCCal.NEWCALLSIGN]);
                Ssutil.DbGetString(hStmt, 2, "oldlocation", out cCalRec.oldcallsign, FeCCal.OLDCALLSIGN_SZ, out nullInd[FeCCal.OLDCALLSIGN]);
            }
            catch (Exception e)
            {
                Log2.e("\n\nDynFeCCal.FeFetchCCal(): ERROR: Exception caught: " + e.Message);
                GenUtil.SetErr("dynfeCCal02 -- Could not get FeCCal object, because of field " + e.Message);
                return Error.ODBC_GET_FAILED;
            }

            //...Log2.v("\n\nDynFeCCal.FeFetchCCal(): Exit");
            return Constant.SUCCESS;
        }

        /// <summary>
        /// Inserts a FeCCal record into the database using column values prescribed 
        /// by the fields of the FeCCal object.
        /// </summary>
        /// <param name="curHandle"> - index of the Cursor object that encapsulates the details of the record insertion.</param>
        /// <param name="pCCal"> - a FeCCal object.</param>
        ...
        public static int FeInsertCCal(int curHandle, FeCCal pCCal, SQLLEN[] nullInd)
        {
            //...Log2.v("\n\nDynCCal.FeInsertCCal(): Entry");
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\n\nDynCCal.FeInsertCCal(): ERROR: call to SQLAllocHandle() failed.");
            ...
            cSQL = FeCCal.BuildSqlInsertString(cursors[curHandle].tableName);
            ...
            //into global memory with an SQLPOINTER pointer assigned to each one. FeCCal provides
            //a convenience method that does exactly this.
            SQLPOINTER[] parameterValuePtr = pCCal.CopyToArrayOfSQLPOINTERinGlobalMemory();
            ...
            try
            {
                ...
                Ssutil.DbBindStringInput(hStmt, 0, "newcallsign", parameterValuePtr[FeCCal.NEWCALLSIGN], (SQLULEN)pCCal.newcallsign.Length, nullIndPtr[FeCCal.NEWCALLSIGN]);

                Ssutil.DbBindStringInput(hStmt, 0, "oldcallsign", parameterValuePtr[FeCCal.OLDCALLSIGN], (SQLULEN)pCCal.oldcallsign.Length, nullIndPtr[FeCCal.OLDCALLSIGN]);
            }
            catch (Exception e)
            {
                Log2.e("\n\nDynCCal.FeInsertCCal(): ERROR: a call to DbBindStringInput() failed.");
                Ssutil.DbGetDiagStmt(hStmt, "feInsertCCal01 -- Error binding parameters for CCal on field: " + e.Message);
                Ssutil.DisConnStmt(hConn, hStmt);
                return Error.ODBC_BINDING_FAILED;
            }

            //...Log2.v("nDynCCal.FeInsertCCal(): SQLExecDirect():\r\n" + cSQL);

            sqlRet = ODBC.SQLExecDirect(hStmt, cSQL, cSQL.Length);

            //...Log2.v("\r\nDynCCal.FeInsertCCal(): SQLExecDirect(): sqlRet = " + sqlRet);

            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\n\nDynCCal.FeInsertCCal(): ERROR: a call to SQLExecDirect() failed for: " + cSQL);
                string str = "feInsertCCal02 -- Error inserting CCal";
                Ssutil.DbGetDiagStmt(hStmt, str); ;
                Ssutil.DisConnStmt(hConn, hStmt);
                return Error.ODBC_EXECUTE_FAILED;
            }

            //...Log2.v("\n\nDynCCal.FeInsertCCal(): successfuly inserted record into table: " + cursors[curHandle].tableName);

            //This method call releases hStmt, disconnects from the DB and then releases hConn.
            Ssutil.DisConnStmt(hConn, hStmt);

            //...Log2.v("\n\nDynCCal.FeInsertCCal(): Exit");
            return Constant.SUCCESS;
        }
