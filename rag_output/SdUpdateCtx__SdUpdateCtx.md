# Documented File: SdUpdateCtx.cs
**Repository Path:** `SdUpdateCtx\SdUpdateCtx.cs`
...
/// <summary>
/// This program updates the Subsidiary Database (SDB) CTX tables <b>main.sd_ctx_</b> and <b>main.sd_ctxd</b> using field
/// values provided in prescribed user SDF files su_XXX_ctx_ and su_XXX_ctxd.
/// </summary>
/// <remarks>
/// The command-line usage is:
/// \image html "Usage - SdUpdateCtx.PNG" ""
/// </remarks>
namespace SdUpdateCtx
{
    ...
    /// <summary>
    /// This class provides the Main() method for the MICS program SdUpdateCtx.
    /// </summary>
    public class SdUpdateCtx
    {
        ...
        /// <summary>
        /// This is the Main() method for the MICS program SdUpdateCtx.
        /// </summary>
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                // user that executed this program (e.g. 'hulme'). Consequently, hulme.sd_ctx_
                // acts as a surrogate for main.sd_ctx_ etc.
                ...
                DynSdbCtx.SetState(Info.MdbWriteEnabled, Info.SpoofModeIsOff, Info.GlobalSchema);
                DynSdbCtxD.SetState(Info.MdbWriteEnabled, Info.SpoofModeIsOff, Info.GlobalSchema);
                ...
                BiUtil.BiBillingRec("UPD_CTX", infoBuffer);
                 ...
                 */
                Info.SetDateTimeNow();
                Console.Write("\t\tSDB CTX UPDATE REPORT Build {0}\t\tDate: {1} {2}\r\n",
                             Info.BuildMetaData, Info.Date, Info.Time);
                Console.Write("\r\n");
                Console.Write("SDF Name: {0}\r\n", Info.SdfName);

                // Check to see that the associated _ctx_ and _ctxd tables for this
                // SDF base name actually exist in the DB.
                // The method call below checks both table types.
                if (!Ssutil.UtTableExist(Constant.SU_CTX, Info.SdfName))
                {
                    Console.Write("\r\n************************************************\r\n");
                    Console.Write("\r\nCTX sdf {0} does not exist\r\n", Info.SdfName);
                    Console.Write("\r\n************************************************\r\n");
                ...
                if (rc != Constant.SUCCESS)
                {
                    ...
                    Console.Write("\r\nsdUpdateCtx: Could not set the database connection to manual commit.\r\n{0}\r\n",
                                    GenUtil.GetUserMess());
                    exitCode = 95;
                    goto cleanUpAndExit;
                }
                ...
                // This method provides the core functionality for SdUpdateCtx.
                // ============================================================
                rc = Updater.ProcessSuCtxCtxD(hConn, Info.SdfName,
                ...
                AtRecordChng.AtRecordChange(Info.DbName, Info.SdfName, Constant.SU_CTX);
            ...
            {
                Log2.e("\r\r\n\nSdUpdateCtx.Main(): exception caught: " + e.Message);
                Log2.e("\r\r\n\nSdUpdateCtx.Main(): stack trace: \r\r\n\n" + e.StackTrace);
                BiUtil.BiBillingRec(Constant.BI_END, "");
                Ssutil.UtDisconnect(1);
        ...
        {
            Console.Write("\r\n");
            Console.Write("\r\n This program updates the Subsidiary Database (SDB) tables main.sd_ctx and main.sd_ctxd using field   +");
            Console.Write("\r\n values provided in prescribed user SDF files [user].su_[sdfName]_ctx_ and [user].su_[sdfName]_ctxd.");
            Console.Write("\r\n");
            Console.Write("\r\n USAGE: SdUpdateCtx <dbName> <projCode> <sdfName> [-c] [-d] [-p] [-s] [-t]");
            Console.Write("\r\n ===== ");
            Console.Write("\r\n");
            Console.Write("\r\n        dbName           : database name, e.g. 'fcsa'.");
            Console.Write("\r\n        project          : user's project 'charge' code.");
            Console.Write("\r\n        sdfName          : the XXX in user table names su_XXX_ctx_ and su_XXX_ctxd.");
            Console.Write("\r\n");
            Console.Write("\r\n        --- For Testing -------------------------------------------------------------");
            ...
            Console.Write("\r\n");
            Console.Write("\r\n e.g.");
            Console.Write("\r\n     SdUpdateCtx fcsa HULME1_0 test_combo01");
            Console.Write("\r\n");
            Console.Write("\r\n Notes:");
        ...
        public static void ParseCommandLineArgs(string[] args)
        {
            ...
            //...Log2.v("\nSdUpdateCtx:" + Info.ToString());
        }

        /// <summary>
        /// This method writes the values of two sets of 'Action' counters to Console.Out - 
        /// one set of counts summarizes the processing of 'master' CTX records in the 
        /// prescribed user su_XXX_ctx_ table and one set of counters summarizes the processing
        /// of the 'detail' records in the corresponding su_XXX_ctxd table; each set of 
