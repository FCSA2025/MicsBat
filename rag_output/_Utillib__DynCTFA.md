# Documented File: DynCTFA.cs
**Repository Path:** `_Utillib\DynCTFA.cs`
...
namespace _Utillib
{
    ...
    /// a Canadian Table of Frequency Allocations (CTFA) SQL table.
    /// </summary>
    public class DynCTFA
    {
        ...
        private static int GetNextFreeCursor()
        {
            ...
            //...Log2.v("\r\nDynCTFA.GetNextFreeCursor(): nNextFreeCursor = " + nNextFreeCursor);
            if (nNextFreeCursor >= Constant.NUM_CURSORS_FEW)
            {
                ...
                {
                    //...Log2.v("\r\nDynCTFA.GetNextFreeCursor(): nInd = " + nInd);
                ...
                if (curHandle == -1)
                {
                    ...
                    GenUtil.SetErr("DynCTFA01 -- No more open cursors.");
                    Application.Exit("\r\nDynCTFA.GetNextFreeCursor(): ERROR: No available cursors.");
                    return -1;
                }
            ...
            //...Log2.v("\r\nDynCTFA.GetNextFreeCursor(): found available cursor: curHandle = " + curHandle);
        ...
        /// <summary>
        /// Selects records from the prescribed CTFA database table i.a.w.
        ...
        /// <returns></returns>
        /// <para>- non-negative value - the index of the cursor to be used for FetchCTFA() calls.</para>
        /// <para>- ErrorMessages.NO_CURSOR_AVAILABLE - reached limit for the number of cursors that can be open concurrently.</para>
        /// <para>-   ErrorMessages.ODBC_EXECUTE_FAILED    - call to ODBC.SQLExecute() failed to return data. </para>
        public static int SelectCTFA(string tableName, string searchCriteria, string orderBy)
        {
            //...Log2.v("\n\nDynCTFA.SelectCTFA(): Entry");
            ...
            if (mhStmt == SQLHANDLE.Zero)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    Log2.e("\nDynCTFA.SelectCTFA(): ERROR : Exit: call to SQLAllocHandle() failed, sqlRet = " + sqlRet);
                    return Error.ODBC_SQLALLOCHANDLE_FAILED;
                }

                CTFA.BindPtrsToCols(mhStmt, out mTgtValPtrs, out mNullIndPtrs);
            }
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\nDynCTFA.SelectCTFA(): ERROR : Exit: Attempt to close the cursor on the reusable statement handle failed.");
            ...
            //...Log2.v("\r\nDynCTFA.SelectCTFA(): stmt_buf = \r\n" + stmt_buf);

            sqlRet = ODBC.SQLExecDirect(mhStmt, stmt_buf, stmt_buf.Length);

            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\nDynCTFA.SelectCTFA(): ERROR : Exit: call to SQLExecDirect() failed, sqlRet = " + sqlRet);
            ...
            //...Log2.v("\n\nDynCTFA.SelectCTFA(): Exit");
            return (curHandle);
        }

        /// <summary>
        /// Retrieves a single equipment record from the previously prescribed CTFA table using 
        /// the cursor object created by a previous call to SelectCTFA().
        /// </summary>
        /// <param name="nCursor"> - FtCursor object.</param>
        /// <param name="ctfa"> - a CTFA object populated with data from the row.</param>
        /// <param name="nullInds"> - array of ODBC nullInds for CTFA object.</param>
        /// <returns></returns>
        ...
        public static int FetchCTFA(int nCursor, out CTFA ctfa, out SQLLEN[] nullInds)
        {
            // 'out' requirments.
            ctfa = null;
            nullInds = null;

            //...Log2.v("\n\nDynCTFA.FetchCTFA(): Entry");
            ...
            {
                Log2.e("\n\nDynCTFA.FetchCTFA(): Exit: ERROR: failed contiguity test.");
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                ...
                {
                    //...Log2.v("\n\nDynCTFA.FetchCTFA(): Exit: sqlRet == ODBC.SQL_NO_DATA");
                    return ODBC.SQL_NO_DATA;
                }
                else
                {
                    // We have an error.
                    Log2.e("\n\nDynCTFA.FetchCTFA(): Exit: ERROR: SQLFetch(): sqlRet = " + sqlRet);
            ...
            else
            {
                ...
                try
                {
                    CTFA.ReadColBindings(mTgtValPtrs, mNullIndPtrs, out ctfa, out nullInds);
                ...
                catch (Exception e)
                {
                    ...
                    Log2.e("\n\nDynCTFA.FetchCTFA(): Exit: ERROR: call to ReadColBindings() threw an exception:\n" + e.Message);
                    return -22;
                } //try-catch

            } //if-else

            //...Log2.v(CTFA.ToStringWN(nullInds));

            //...Log2.v("\n\nDynCTFA.FetchCTFA(): Exit");
            return nRet;
        } //end of FetchCTFA

        /// <summary>
        /// Closes (releases) a cursor instantiated by a previous call to SelectCTFA(); 
        ...
        /// <para>-   ErrorMessages.DYN_CUR_NOT_OPEN  - FtCursor field cursorOpen is set to false.</para>
        public static int CloseCTFA(int curHandle)
        {
        ...
        /// <summary>
        /// This method returns a populated CTFA object and associated ODBC nullInds
        /// corresponding to the record in the prescribed CTFA table that has the 
        /// prescribed key ('keyfield').
        /// </summary>
        /// <param name="tableName"> - fully-qualified SQL table name.</param>
        /// <param name="keyField"></param>
        /// <param name="CTFA"></param>
        /// <param name="nullInds"></param>
        /// <returns>True if a record with the prescribed key was found in the table; otherwise false.</returns>
        public static bool FetchCTFARecordWithKey(string tableName, string keyField, out CTFA CTFA, out SQLLEN[] nullInds)
        {
            // 'out' requirements.
            CTFA = null;
            nullInds = null;
            ...
            // Do the 'SELECT'.
            int nCursor = SelectCTFA(tableName, whereClause, "");

            if (nCursor < 0)
            {
                Log2.e("\nDynCTFA.FetchCTFARecordWithKey(): ERROR: Select() failed, nCursor = " + nCursor);
                return false;
            }

            // Do the 'FETCH'.
            int rc = FetchCTFA(nCursor, out CTFA, out nullInds);
            ...
            {
                Log2.e("\nDynCTFA.FetchRecordWithKey(): ERROR: Fetch() failed, rc = " + rc);
                isSuccessful = false;
            }
        ...
        /// <summary>
        /// Inserts a CTFA record into the database using column values prescribed 
        /// by the fields of the CTFA object.
        /// </summary>
        /// <param name="tableName"> - name of the SQL table to INSERT into.</param>
        /// <param name="CTFA"> - a FtSite object.</param>
        ...
        public static int InsertCTFA(string tableName, CTFA CTFA, SQLLEN[] nullInd)
        {
            //...Log2.v("\n\nDynCTFA.InsertCTFA(): Entry");
            ...
            update_buf = CTFA.BuildSqlInsertString(tableName);
            ...
            SQLPOINTER[] parameterValuePtr = CTFA.CopyToArrayOfSQLPOINTERinGlobalMemory();
            ...
            {
                colNum = 1;
                Ssutil.DbBindDoubleInput(hStmt, colNum, "lowerFreqKHz", parameterValuePtr[colNum - 1], nullIndPtr[colNum - 1]);

                colNum = 2;
                Ssutil.DbBindDoubleInput(hStmt, colNum, "upperFreqKHz", parameterValuePtr[colNum - 1], nullIndPtr[colNum - 1]);

                colNum = 3;
                Ssutil.DbBindStringInput(hStmt, colNum, "fixed", parameterValuePtr[colNum - 1], (SQLULEN)CTFA.Fixed.Length, nullIndPtr[colNum - 1]);

                colNum = 4;
                Ssutil.DbBindStringInput(hStmt, colNum, "earthToSatellite", parameterValuePtr[colNum - 1], (SQLULEN)CTFA.EarthToSatellite.Length, nullIndPtr[colNum - 1]);

                colNum = 5;
                Ssutil.DbBindStringInput(hStmt, colNum, "satelliteToEarth", parameterValuePtr[colNum - 1], (SQLULEN)CTFA.SatelliteToEarth.Length, nullIndPtr[colNum - 1]);

                colNum = 6;
                Ssutil.DbBindStringInput(hStmt, colNum, "srspID", parameterValuePtr[colNum - 1], (SQLULEN)CTFA.SrspID.Length, nullIndPtr[colNum - 1]);
            }
            catch
            {
                Log2.e("\n\nDynCTFA.InsertCTFA(): ERROR: call to Ssutil.DbBindXXXInput() failed for colNum = {0}", colNum);
                Ssutil.DisConnStmt(hConn, hStmt);
                return -1;
            }

            //...Log2.v("nDynCTFA.InsertCTFA(): SQLExecDirect():\r\n" + update_buf);
            SQLRETURN sqlRet;
            sqlRet = ODBC.SQLExecDirect(hStmt, update_buf, update_buf.Length);

            //...Log2.v("\r\nDynCTFA.InsertCTFA(): SQLExecDirect(): sqlRet = " + sqlRet);

            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\n\nDynCTFA.InsertCTFA(): ERROR: call to SQLExecDirect failed for query:\n{0}", update_buf);
