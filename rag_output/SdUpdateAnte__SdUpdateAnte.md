# Documented File: SdUpdateAnte.cs
**Repository Path:** `SdUpdateAnte\SdUpdateAnte.cs`
...
/// <summary>
/// This program updates the Subsidiary Database (SDB) tables <b>main.sd_ante</b> and <b>main.sd_antd</b> using field
/// values provided in prescribed user SDF files su_XXX_ante and su_XXX_antd.
/// </summary>
/// <remarks>
/// The command-line usage is:
/// \image html "Usage - SdUpdateAnte.PNG" ""
/// </remarks>
namespace SdUpdateAnte
{
    ...
    /// <summary>
    /// This class provides the Main() method for the MICS program SdUpdateAnte.
    /// </summary>
    public class SdUpdateAnte
    {
        private static bool mIsCreateSpoofTables = false;
        private static bool mIsWriteSpoofTestData = false;

        /// <summary>
        /// This is the Main() method for the MICS program SdUpdateAnte.
        /// </summary>
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                // user that executed this program (e.g. 'hulme'). Consequently, hulme.sd_ante
                // acts as a surrogate for main.sd_ante etc.
                ...
                DynSdbAnte.SetState(Info.MdbWriteEnabled, Info.SpoofModeIsOff, Info.GlobalSchema);
                DynSdbAntd.SetState(Info.MdbWriteEnabled, Info.SpoofModeIsOff, Info.GlobalSchema);
                ...
                BiUtil.BiBillingRec("UPD_ANTE", infoBuffer);
                 ...
                 */
                Info.SetDateTimeNow();
                Console.Write("\t\tSDB UPDATE REPORT Build {0}\t\tDate: {1} {2}\r\n",
                             Info.BuildMetaData, Info.Date, Info.Time);
                Console.Write("\r\n");
                Console.Write("SDF Name: {0}\r\n", Info.SdfName);

                // Check to see that the associated _ante and _antd tables for this
                // SDF base name actually exist in the DB.
                // The method call below checks both table types.
                if (!Ssutil.UtTableExist(Constant.SU_ANTE, Info.SdfName))
                {
                    Console.Write("\r\n************************************************\r\n");
                    Console.Write("\r\nAntenna sdf {0} does not exist\r\n", Info.SdfName);
                    Console.Write("\r\n************************************************\r\n");
                ...
                Console.Write("SDF Type: Antenna\r\n\r\n");
                ...
                if (rc != Constant.SUCCESS)
                {
                    ...
                    Console.Write("\r\nsdUpdateAnte: Could not set the database connection to manual commit.\r\n{0}\r\n",
                                    GenUtil.GetUserMess());
                    exitCode = 95;
                    goto cleanUpAndExit;
                }
                ...
                status = Validation.Perform(out userTables, out userTablesNullInds);
                ...
                // This method provides the core functionality for SdUpdateAnte.
                // ============================================================
                rc = Updater.ProcessSuAnteAntd(hConn, Info.SdfName,
                ...
                AtRecordChng.AtRecordChange(Info.DbName, Info.SdfName, Constant.SU_ANTE);
            ...
            {
                Log2.e("\r\r\n\nSdUpdateAnte.Main(): exception caught: " + e.Message);
                Log2.e("\r\r\n\nSdUpdateAnte.Main(): stack trace: \r\r\n\n" + e.StackTrace);
                BiUtil.BiBillingRec(Constant.BI_END, "");
                Ssutil.UtDisconnect(1);
        ...
        {
            Console.Write("\r\n");
            Console.Write("\r\n This program updates the Subsidiary Database (SDB) tables main.sd_ante and main.sd_antd using field   +");
            Console.Write("\r\n values provided in prescribed user SDF files [user].su_[sdfName]_ante and [user].su_[sdfName]_antd.");
            Console.Write("\r\n");
            Console.Write("\r\n USAGE: SdUpdateAnte <dbName> <projCode> <sdfName> [-c] [-d] [-s] [-t]");
            Console.Write("\r\n ===== ");
            Console.Write("\r\n");
            Console.Write("\r\n        dbName           : database name, e.g. 'fcsa'.");
            Console.Write("\r\n        project          : user's project 'charge' code.");
            Console.Write("\r\n        sdfName          : the XXX in user table names su_XXX_ante and su_XXX_antd.");
            Console.Write("\r\n");
            Console.Write("\r\n        --- For Testing -------------------------------------------------------------");
            ...
            Console.Write("\r\n");
            Console.Write("\r\n e.g.");
            Console.Write("\r\n     SdUpdateAnte fcsa HULME1_0 test_combo01");
            Console.Write("\r\n");
            Console.Write("\r\n Notes:");
        ...
        public static void ParseCommandLineArgs(string[] args)
        {
            ...
            //...Log2.v("\nSdUpdateAnte:" + Info.ToString());
        }

        /// <summary>
        /// This method writes the values of two sets of 'Action' counters to Console.Out - 
        /// one set of counts summarizes the processing of 'master' antenna records in the 
        /// prescribed user su_XXX_ante table and one set of counters summarizes the processing
        /// of the 'detail' records in the corresponding su_XXX_antd table; each set of 
        ...
        {
            Console.Write("\r\n\r\n\t************ Summary Information for SDB Antenna Update ****\r\n");
