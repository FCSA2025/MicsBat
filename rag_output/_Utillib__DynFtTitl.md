# Documented File: DynFtTitl.cs
**Repository Path:** `_Utillib\DynFtTitl.cs`
...
namespace _Utillib
{
    ...
    public class DynFtTitl
    {
        ...
        public static int GetNextFreeCursor()
        {
            ...
            if (nNextFreeCursor >= Constant.NUM_CURSORS_FEW)
            {
                ...
                if (curHandle == -1)
                {
                    ...
                    //return -1;
                    Application.Exit("\r\nDynFtTitl.GetNextFreeCursor(): ERROR: No available cursors.");
                }
            }
            else
            {
        ...
        /// <returns></returns>
        /// <para>- non-negative value - the index of the cursor to be used for FtFetchTitl() calls.</para>
        /// <para>- ErrorMessages.NO_CURSOR_AVAILABLE - reached limit for the number of cursors that can be open concurrently.</para>
        /// <para>-   ErrorMessages.ODBC_EXECUTE_FAILED    - call to ODBC.SQLExecute() failed to return data. </para>
        public static int FtSelectTitl(string table, string searchCriteria, string orderBy)
        {
            //...Log2.v("\n\nDynFtTitl.FtSelectTitle(): Entry");
            SQLHANDLE hStmt;
            SQLRETURN sqlRet;
            int curHandle;          /* cursor handle */

            //Construct the select clause.
            string cSQL = "select " + FtTitl.AllColumnsForSqlSelect + " from " + table;
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("DynFtTitl.FtSelectTitle(): ERROR: call to SQLExecDirect() failed for: " + cSQL);
        ...
        public static int FtCloseTitl(int curHandle)
        {
        ...
        /// <param name="titlRec"> - a FtTitl object populated with data from the row.</param>
        ...
        public static int FtFetchTitl(int curHandle, out FtTitl titlRec, out SQLLEN[] nullInd)
        {
            //...Log2.v("\n\nDynFtTitl.FtFetchTitl(): Entry");

            //Create an FtTitl object to output.
            titlRec = new FtTitl();
            //Create an array of nulls to output.
            nullInd = new SQLLEN[FtTitl.NUM_COLUMNS];
            ...
            if (!cursors[curHandle].cursorOpen)
            {
                Log2.e("\r\nDynFtTitl.FtFetchTitl(): ERROR: if (!cursors[curHandle].cursorOpen)");
                return (Error.DYN_CUR_NOT_OPEN);
            }
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                ...
                {
                    //...Log2.v("\r\nDynFtTitl.FtFetchTitl(): ODBC.SQLFetch(): sqlRet == ODBC.SQL_NO_DATA");
                    return (int)sqlRet;
                }
                else
                {
                    //	Error
                    //...Log2.v("\r\nDynFtTitl.FtFetchTitl(): ODBC.SQLFetch(): FAIL: sqlRet = " + sqlRet);
            ...
            try
            {
                Ssutil.DbGetString(hStmt, 1, "validated", out titlRec.validated, FtTitl.VALIDATED_SZ, out nullInd[FtTitl.VALIDATED]);

                Ssutil.DbGetString(hStmt, 2, "namef", out titlRec.namef, FtTitl.NAMEF_SZ, out nullInd[FtTitl.NAMEF]);

                Ssutil.DbGetString(hStmt, 3, "source", out titlRec.source, FtTitl.SOURCE_SZ, out nullInd[FtTitl.SOURCE]);

                Ssutil.DbGetString(hStmt, 4, "descr", out titlRec.descr, FtTitl.DESCR_SZ, out nullInd[FtTitl.DESCR]);

                Ssutil.DbGetString(hStmt, 5, "mdate", out titlRec.mdate, FtTitl.MDATE_SZ, out nullInd[FtTitl.MDATE]);

                Ssutil.DbGetString(hStmt, 6, "mtime", out titlRec.mtime, FtTitl.MTIME_SZ, out nullInd[FtTitl.MTIME]);
            }
            catch (Exception e)
            {
                Log2.e("\r\nDynFtTitl.FtFetchTitl(): ERROR: Exception caught: " + e.Message);
                GenUtil.SetErr("FtFetchTitl02: Input error on field: " + e.Message);
                return Error.ODBC_GET_FAILED;
            }

            //...Log2.v("\n\nDynFtTitl.FtFetchTitl(): Exit");
            return Constant.SUCCESS;
        }

        /// <summary>
        /// Inserts a 'titl' record into the database using column values prescribed 
        /// by the fields of the FtTitl object.
        /// </summary>
        /// <param name="curHandle"> - index of the Cursor object that encapsulates the details of the record insertion.</param>
        /// <param name="pTitl"> - a FtTitl object.</param>
        ...
        public static int FtInsertTitl(int curHandle, FtTitl pTitl, SQLLEN[] nullInd)
        {
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\n\nDynTitl.FtInsertTitl(): ERROR: call to SQLAllocHandle() failed.");
            ...
            cSQL = FtTitl.BuildSqlInsertString(cursors[curHandle].tableName);
            ...
            //into global memory with an SQLPOINTER pointer assigned to each one. FtTitl provides
            ...
            try
            {
                ...
                Ssutil.DbBindStringInput(hStmt, FtTitl.VALIDATED + 1, "validated", parameterValuePtr[FtTitl.VALIDATED], (SQLULEN)pTitl.validated.Length, nullIndPtr[FtTitl.VALIDATED]);

                Ssutil.DbBindStringInput(hStmt, FtTitl.NAMEF + 1, "namef", parameterValuePtr[FtTitl.NAMEF], (SQLULEN)pTitl.namef.Length, nullIndPtr[FtTitl.NAMEF]);

                Ssutil.DbBindStringInput(hStmt, FtTitl.SOURCE + 1, "source", parameterValuePtr[FtTitl.SOURCE], (SQLULEN)pTitl.source.Length, nullIndPtr[FtTitl.SOURCE]);

                Ssutil.DbBindStringInput(hStmt, FtTitl.DESCR + 1, "descr", parameterValuePtr[FtTitl.DESCR], (SQLULEN)pTitl.descr.Length, nullIndPtr[FtTitl.DESCR]);

                Ssutil.DbBindStringInput(hStmt, FtTitl.MDATE + 1, "mdate", parameterValuePtr[FtTitl.MDATE], (SQLULEN)pTitl.mdate.Length, nullIndPtr[FtTitl.MDATE]);

                Ssutil.DbBindStringInput(hStmt, FtTitl.MTIME + 1, "mtime", parameterValuePtr[FtTitl.MTIME], (SQLULEN)pTitl.mtime.Length, nullIndPtr[FtTitl.MTIME]);

            }
            catch (Exception e)
            {
                Log2.e("\n\nDynTitl.FtInsertTitl(): ERROR: a call to DbBindStringInput() failed: " + e.Message);
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\n\nDynTitl.FtInsertTitl(): ERROR: a call to SQLExecDirect() failed for: " + cSQL);
            ...
            //...Log2.v("\nDynTitl.FtInsertTitl(): successfuly inserted record into table: " + cursors[curHandle].tableName);
        ...
        /// </summary>
        /// <param name="curHandle"> - a prescribed Cursor object, as returned by a previous call to FtSelectTitl().</param>
        /// <param name="ftTitl"> - a prescribed FtTitl object that provides the updated field values.</param>
        /// <param name="nullInd"> - an array of ODBC nullInds associated with feTitl.</param>
        /// <returns></returns>
        public static int FtUpdateTitl(int curHandle,		/* handle returned by 'feSelectTitl */
                                        FtTitl ftTitl,        /* caller's struct for row of data */
                                        SQLLEN[] nullInd)   /* caller's array of null indicators */
        {
            FtDeleteTitl(curHandle);

            return FtInsertTitl(curHandle, ftTitl, nullInd);
        }
        ...
        /// <param name="curHandle"> - a prescribed Cursor object, as returned by a previous call to FtSelectTitl().</param>
        /// <returns></returns>
        public static int FtDeleteTitl(int curHandle)
        {
            string delete_buf;
            SQLRETURN sqlRet;
            SQLHANDLE hConn = Ssutil.NewConn();
