# Documented File: SuDynTown.cs
**Repository Path:** `_Utillib\SuDynTown.cs`
...
namespace _Utillib
{
    ...
    /// <summary>
    /// This class provides methods that serve SuTown objects to/from a database.
    /// </summary>
    public class SuDynTown
    {
        //===================================================================================================================================
        ...
        public static int GetNextFreeCursor()
        {
            ...
            if (nNextFreeCursor >= Constant.NUM_CURSORS_FEW)
            {
                ...
                if (curHandle == -1)
                {
                    Application.Exit("\r\nSuDynTown.GetNextFreeCursor(): ERROR: No available cursors.");
                }
            }
            else
            {
        ...
        /// <summary>
        /// Selects SuTown records from a user's su_XXX_town DB table for
        ...
        /// <returns></returns>
        /// <para>- non-negative value - the index of the cursor to be used for SuFetchTown() calls.</para>
        /// <para>- ErrorMessages.NO_CURSOR_AVAILABLE - reached limit for the number of cursors that can be open concurrently.</para>
        /// <para>-   ErrorMessages.ODBC_EXECDIRECT_FAILED    - call to ODBC.SQLExecute() failed to return data. </para>
        public static int SuSelectTown(string table, string searchCriteria, string orderBy)
        {
            //...Log2.v("\n\nSuDynTown.SuSelectTown(): Entry");
            SQLHANDLE hStmt;
            SQLHANDLE hUpdate;
            SQLRETURN sqlRet;
            ...
            string stmt_buf = "SELECT " + SuTown.AllColumnsForSqlSelect + " FROM " + table;
            ...
            //...Log2.v("\r\nSuDynTown.SuSelectTown(): [1] sqlRet = " + sqlRet);

            //...Log2.v("\nSuDynTown.SuSelectTown(): query = " + stmt_buf);

            sqlRet = ODBC.SQLExecDirect(hStmt, stmt_buf, ODBC.SQL_NTS);

            //...Log2.v("\r\nSuDynTown.SuSelectTown(): [4] sqlRet = " + sqlRet);

            if (!ODBC.IsOK(sqlRet))
            {
                Ssutil.DbGetDiagStmt(hStmt, "Could not Select Town for criteria: " + searchCriteria + ".");
                ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                Ssutil.DisConn(hConn);
                Log2.e("\nSuDynTown.SuSelectTown(): ERROR: SQLExecute() failed");
            ...
            //...Log2.v("\n\nSuDynTown.SuSelectTown(): Exit");
            return curHandle;
        }

        /// <summary>
        /// Retrieves a single row of SuTown data from a table in the database using 
        /// a Cursor object previously created by a call to SuSelectTown().
        /// </summary>
        /// <param name="curHandle"> - index of a Cursor object.</param>
        /// <param name="suTown"> - a SuTown object populated with data from the row.</param>
        /// <param name="nullInd"> - array of ODBC nullInds for suTown.</param>
        /// <returns></returns>
        ...
        public static short SuFetchTown(int curHandle, out SuTown suTown, out SQLLEN[] nullInd)
        {
            //...Log2.v("\n\nSuDynTown.SuFetchTown(): Entry:");

            //Create an SuTown object to output.
            suTown = new SuTown();
            //Create an array of nulls to output.
            nullInd = new SQLLEN[SuTown.NUM_COLUMNS];
            ...
            if (!cursors[curHandle].cursorOpen)
            {
                ...
                Log2.e("\n\nSuDynTown.SuFetchTown(): Exit: FAIL: if (!cursors[curHandle].cursorOpen)");
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                ...
                {
                    //...Log2.v("\n\nSuDynTown.SuFetchTown(): Exit: ODBC.SQLFetch(): sqlRet == ODBC.SQL_NO_DATA");
                    return sqlRet;
                }
                else
                {
                    //	Error
                    Log2.e("\n\nSuDynTown.SuFetchTown(): Exit: ODBC.SQLFetch(): FAIL: sqlRet = " + sqlRet);
            ...
            try
            {
                Ssutil.DbGetString(hStmt, 1, "cmd", out suTown.cmd, SuTown.CMD_SZ, out nullInd[SuTown.CMD]);
                Ssutil.DbGetString(hStmt, 2, "recstat", out suTown.recstat, SuTown.RECSTAT_SZ, out nullInd[SuTown.RECSTAT]);
                Ssutil.DbGetString(hStmt, 3, "call1", out suTown.call1, SuTown.CALL1_SZ, out nullInd[SuTown.CALL1]);
                Ssutil.DbGetString(hStmt, 4, "oper", out suTown.oper, SuTown.OPER_SZ, out nullInd[SuTown.OPER]);
                Ssutil.DbGetString(hStmt, 5, "twcode", out suTown.twcode, SuTown.TWCODE_SZ, out nullInd[SuTown.TWCODE]);
                Ssutil.DbGetFloat(hStmt, 6, "twht", out suTown.twht, out nullInd[SuTown.TWHT]);
                Ssutil.DbGetByte(hStmt, 7, "atwrno", out suTown.atwrno, out nullInd[SuTown.ATWRNO]);
                Ssutil.DbGetString(hStmt, 8, "twli", out suTown.twli, SuTown.TWLI_SZ, out nullInd[SuTown.TWLI]);
                Ssutil.DbGetString(hStmt, 9, "twpa", out suTown.twpa, SuTown.TWPA_SZ, out nullInd[SuTown.TWPA]);
                Ssutil.DbGetString(hStmt, 10, "nott", out suTown.nott, SuTown.NOTT_SZ, out nullInd[SuTown.NOTT]);
                Ssutil.DbGetString(hStmt, 11, "tpoint", out suTown.tpoint, SuTown.TPOINT_SZ, out nullInd[SuTown.TPOINT]);
                Ssutil.DbGetString(hStmt, 12, "adate", out suTown.adate, SuTown.ADATE_SZ, out nullInd[SuTown.ADATE]);
                Ssutil.DbGetString(hStmt, 13, "sdate", out suTown.sdate, SuTown.SDATE_SZ, out nullInd[SuTown.SDATE]);
                Ssutil.DbGetString(hStmt, 14, "mdate", out suTown.mdate, SuTown.MDATE_SZ, out nullInd[SuTown.MDATE]);
                Ssutil.DbGetString(hStmt, 15, "mtime", out suTown.mtime, SuTown.MTIME_SZ, out nullInd[SuTown.MTIME]);
            }
            catch (Exception e)
            {
                Log2.e("\n\nSuDynTown.SuFetchTown(): ERROR: Exception caught: " + e.Message);
                GenUtil.SetErr("dynfeTown02 -- Could not get town, because of field " + e.Message);
                return Error.ODBC_GET_FAILED;
            }

            //...Log2.v("\n\nSuDynTown.SuFetchTown(): Exit");
        ...
        /// <summary>
        /// Closes (releases) a cursor instantiated by a previous call to SuSelectTown(); 
        ...
        public static int SuCloseTown(int curHandle)
        {
