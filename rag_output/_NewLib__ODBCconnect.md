# Documented File: ODBCconnect.cs
**Repository Path:** `_NewLib\ODBCconnect.cs`
...
namespace _NewLib
{
    ...
    public class ODBCconnect
    {
        ...
        public static int UtConnect(string dbNameIn)
        {
            int rc;
            ...
            {
                //...Console.Error.Write("\nODBCconnect.UtConnect(): Error: dbNameIn is null, empty or whitespace");
                return Error.NODATABASE;
            }

            // Check if a connection has already been established; if so, return with a fail code.
            if (mStaticConnHandle != SQLHANDLE.Zero)
            {
                //...Console.Error.Write("\nODBCconnect.UtConnect(): Error: method called when there is already an existing ODBC connection.");
                return Error.INVPARM;
            }
            ...
            if (rc != 0)
            {
                //...Console.Error.Write("\nODBCconnect.UtConnect(): Error: call to DbGetEnv() failed: unable to get environment handle.");
                return Error.ODBC_COULD_NOT_GET_ENV_HANDLE;
            }
            ...
            try
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    //...Console.Error.Write("\nODBCconnect.UtConnect(): Error A: call to ODBC.SQLAllocHandle() failed.");
                ...
                if (mIsFirstConnect)
                {
                    ...
                    {
                        //...Console.Error.Write("\nODBCconnect.UtConnect(): Error: Info.MicsUserName not set.");
                        return Constant.FAILURE;
                    }

                    if (String.IsNullOrWhiteSpace(Info.Password))
                    {
                        //...Console.Error.Write("\nODBCconnect.UtConnect(): Error: Info.Password not set.");
                        return Constant.FAILURE;
                    }
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    //...Console.Error.Write("\nODBCconnect.UtConnect(): Error: call to ODBC.SQLConnect() failed.");
                    return Constant.FAILURE;
                }
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    //...Console.Error.Write("\nODBCconnect.UtConnect(): Error B: call to ODBC.SQLAllocHandle() failed.");
                    return Constant.FAILURE;
                }
                ...
                if (String.IsNullOrWhiteSpace(Info.GlobalSchema))
                {
                    ...
                    {
                        //...Console.Error.Write("\nODBCconnect.UtConnect(): Error: call to GetUltrixID() failed: unable to determine user's SQL schema.");
                        return Constant.FAILURE;
                    }
        ...
        public static int GetSystemId(out string cSystemId, SQLULEN nLen)
        {
            ...
            if (String.IsNullOrWhiteSpace(Info.MicsUserName))
            {
                //...Console.Error.Write("\nODBCconnect.GetSystemId(): Error: Info.MicsUserName not set.");
                return Constant.FAILURE;
            }
