# Documented File: DynSdbAntd.cs
**Repository Path:** `_Utillib\DynSdbAntd.cs`
...
namespace _Utillib
{
    ...
    /// <b>main.sd_antd</b>.
    /// </summary>
    public class DynSdbAntd
    {
        private const string mBaseTableName = "sd_antd";
        ...
        private const string mUPDATE = "UPDATE {0} SET {1} WHERE acode='{2}' AND antang='{3}' ";

        private const string mINSERT = "INSERT INTO {0} VALUES ({1}) ";

        private const string mDELETE = "DELETE FROM {0} WHERE acode='{1}' AND antang='{2}' ";

        //private const string mCHNG_UPDATE = "UPDATE {0} SET bndcde='{1}', mdate='{2}', mtime='{3}' WHERE acode='{4}' ";
        ...
        /// <summary>
        /// This method can be used to set whether the SDB <b>main.sd_antd</b> table should be written to,
        /// (SQL DELETE, INSERT and/or UPDATE queries) or not; it can also set whether the SQL queries should
        /// target the actual main.sd_antd table or a user's 'spoof' table (e.g. hulme.sd_antd).
        /// </summary>
        /// <param name="mdbWriteEnabled"> - boolean that prescribes whether sd_antd record
        ...
        public static void SetState(bool mdbWriteEnabled, bool spoofModeIsOff, string userSchema)
        {
            ...
            else
            {
                ...
                {
                    Log2.e("\nDynSdbAntd.SetState(): ERROR: spoofMode is ON but userSchema is null or whitespace.");
        ...
        private static int GetNextFreeCursor()
        {
            ...
            //...Log2.v("\r\nDynSdbAntd.GetNextFreeCursor(): nNextFreeCursor = " + nNextFreeCursor);
            if (nNextFreeCursor >= Constant.NUM_CURSORS_FEW)
            {
                ...
                {
                    //...Log2.v("\r\nDynSdbAntd.GetNextFreeCursor(): nInd = " + nInd);
                ...
                if (curHandle == -1)
                {
                    ...
                    GenUtil.SetErr("DynSdbAntd01 -- No more open cursors.");
                    Application.Exit("\r\nDynSdbAntd.GetNextFreeCursor(): ERROR: No available cursors.");
                    return -1;
                }
            ...
            //...Log2.v("\r\nDynSdbAntd.GetNextFreeCursor(): found available cursor: curHandle = " + curHandle);
        ...
        /// <summary>
        /// Selects antenna records from the database table <b>main.sd_antd</b> for
        ...
        /// <returns></returns>
        /// <para>- non-negative value - the index of the cursor to be used for SdFetchAntd() calls.</para>
        /// <para>- ErrorMessages.NO_CURSOR_AVAILABLE - reached limit for the number of cursors that can be open concurrently.</para>
        /// <para>-   ErrorMessages.ODBC_EXECUTE_FAILED    - call to ODBC.SQLExecute() failed to return data. </para>
        public static int SdSelectAntd(string searchCriteria, string orderBy)
        {
            //...Log2.v("\n\nDynSdbAntd.SdSelectAntd(): Entry");
            ...
            if (mhStmt == SQLHANDLE.Zero)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    Log2.e("\nDynSdbAntd.SdSelectAntd(): ERROR : Exit: call to SQLAllocHandle() failed, sqlRet = " + sqlRet);
                    return Error.ODBC_SQLALLOCHANDLE_FAILED;
                }

                SdAntd.BindPtrsToCols(mhStmt, out mTgtValPtrs, out mNullIndPtrs);
            }
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\nDynSdbAntd.SdSelectAntd(): ERROR : Exit: Attempt to close the cursor on the reusable statement handle failed.");
            ...
            //...Log2.v("\r\nDynSdbAntd.SdSelectAntd(): stmt_buf = \r\n" + stmt_buf);

            sqlRet = ODBC.SQLExecDirect(mhStmt, stmt_buf, stmt_buf.Length);

            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\nDynSdbAntd.SdSelectAntd(): ERROR : Exit: call to SQLExecDirect() failed, sqlRet = " + sqlRet);
            ...
            //...Log2.v("\n\nDynSdbAntd.SdSelectAntd(): Exit");
            return (curHandle);
        }

        /// <summary>
        /// Retrieves a single row of antenna data from the database table <b>main.sd_antd</b> using 
        /// the cursor object created by a previous call to SdSelectAntd().
        /// </summary>
        /// <param name="nCursor"> - FtCursor object.</param>
        /// <param name="sdAntd"> - a SdAntd object populated with data from the row.</param>
        /// <param name="nullInds"> - array of ODBC nullInds for sdAntd.</param>
        /// <returns></returns>
        ...
        public static int SdFetchAntd(int nCursor, out SdAntd sdAntd, out SQLLEN[] nullInds)
        {
            // 'out' requirments.
            sdAntd = null;
            nullInds = null;

            //...Log2.v("\n\nDynSdbAntd.SdFetchAntd(): Entry");
            ...
            {
                Log2.e("\n\nDynSdbAntd.SdFetchAntd(): Exit: ERROR: failed contiguity test.");

                return -666;
            }

            // Fetch the next row of selected Antd data.
            SQLRETURN sqlRet = ODBC.SQLFetch(mhStmt);

            // Check the SQLRETURN and act accordingly.
            if (!ODBC.IsOK(sqlRet))
            {
                // If no more Antd records available then break out the while-loop.
                if (sqlRet == ODBC.SQL_NO_DATA)
                {
                    //...Log2.v("\n\nDynSdbAntd.SdFetchAntd(): Exit: sqlRet == ODBC.SQL_NO_DATA");
                ...
                {
                    // We have an error.
                    Log2.e("\n\nDynSdbAntd.SdFetchAntd(): Exit: ERROR: SQLFetch(): sqlRet = " + sqlRet);
            ...
            else
            {
                ...
                try
                {
                    SdAntd.ReadColBindings(mTgtValPtrs, mNullIndPtrs, out sdAntd, out nullInds);
                ...
                catch (Exception e)
                {
                    ...
                    Log2.e("\n\nDynSdbAntd.SdFetchAntd(): Exit: ERROR: call to ReadColBindings() threw an exception:\n" + e.Message);
                    return -22;
                } //try-catch

            } //if-else

            //...Log2.v(sdAntd.ToStringWN(nullInds));

            //...Log2.v("\n\nDynSdbAntd.SdFetchAntd(): Exit");
            return nRet;
        } //end of SdFetchAntd
        ...
        /// <para>-   ErrorMessages.DYN_CUR_NOT_OPEN  - FtCursor field cursorOpen is set to false.</para>
        public static int SdCloseAntd(int curHandle)
        {
        ...
        /// <param name="sdAntd"> - a prescribed MeAntd object that provides the updated field values.</param>
        /// <param name="nullInds"> - an array of ODBC nullInds associated with sdAntd.</param>
        /// <returns></returns>
        public static int SdUpdateAntd(SQLHANDLE hConn, SdAntd sdAntd, SQLLEN[] nullInds)
        {
            //...Log2.v("\nDynSdbAntd.SdUpdateAntd(): Entry");
            ...
            update_buf = String.Format(mUPDATE, mTableName, sdAntd.ToStringAsCSVequates(nullInds), sdAntd.acode, sdAntd.antang);

            //...Log2.v("\nDynSdbAntd.SdUpdateAntd(): query = " + update_buf);
            ...
            if (mMdbWriteEnabled)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    string str = String.Format("\nDynSdbAntd.SdUpdateAntd(): ERROR: SQLExecDirect failed, sqlRet = {0}, SQL query:\n{1}", sqlRet, update_buf);
                    Log2.e(str);
                    str = String.Format("sdUpdateAntd04 -- Error writing Antd: {0}", sdAntd.acode);
            ...
            {
                Info.Text += "\r\n\r\n" + update_buf;
                //...Log2.v("\nDynSdbAntd.SdUpdateAntd(): Updating of main.sd_antd records is DISABLED.");
            }

            ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);

            //...Log2.v("\nDynSdbAntd.SdUpdateAntd(): Exit");
            return (Constant.SUCCESS);
        }

        /// <summary>
        /// Inserts a record into the SDB table main.sd_antd using column values prescribed 
        /// by the fields of an SdAntd object.
        /// </summary>
        /// <param name="hConn"> - an open ODBC connection handle.</param>
        /// <param name="sdAntd"> - a MeAntd object.</param>
        /// <param name="nullInd"> - array of ODBC nullInds for sdAntd</param>
        /// <returns></returns>
        ...
        public static int SdInsertAntd(SQLHANDLE hConn, SdAntd sdAntd, SQLLEN[] nullInd)
        {
            //...Log2.v("\n\nDynSdbAntd.SdInsertAntd(): Entry");
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\n\nDynSdbAntd.SdInsertAntd(): ERROR: call to SQLAllocHandle() failed.");
                return Error.ODBC_SQLALLOCHANDLE_FAILED;
            }

            //Create the SQL insert statement.
            cSQL = String.Format(mINSERT, mTableName, sdAntd.ToStringAsCSV(nullInd));

            //...Log2.v("nDynMeAntd.MeInsertAntd(): SQLExecDirect():\r\n" + cSQL);
            ...
            if (mMdbWriteEnabled)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    Log2.e("\n\nDynSdbAntd.SdInsertAntd(): ERROR: a call to SQLExecDirect() failed for: " + cSQL);
                    string str = "sdInsertAntd02 -- Error inserting antenna";
                    Ssutil.DbGetDiagStmt(hStmt, str); ;
                    ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                    return Error.ODBC_EXECDIRECT_FAILED;
                }

                //...Log2.v("\n\nDynSdbAntd.SdInsertAntd(): successfuly inserted record into table main.sd_antd");
            }
            else
            {
                Info.Text += "\r\n\r\n" + cSQL;
                //...Log2.v("\nDynSdbAntd.SdInsertAntd(): Insertion of main.sd_antd records is DISABLED.");
            }

            //This method call releases hStmt, disconnects from the DB and then releases hConn.
            ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);

            //...Log2.v("\n\nDynSdbAntd.SdInsertAntd(): Exit");
            return Constant.SUCCESS;
        }

        /// <summary>
        /// This method deletes a record with a prescribed bndcde from the SDB
        /// antenna table main.sd_antd.
        /// </summary>
        /// <param name="hConn"> - an open ODBC connection handle.</param>
        /// <param name="acode"> - a prescribed antenna code.</param>
        /// <param name="antang"> - a prescribed antang floating-point value.</param>
        /// <returns> - the number of records deleted; a negative return value indicates failure.</returns>
        public static int SdDeleteAntd(SQLHANDLE hConn, string acode, float antang)
        {
            //...Log2.v("\n\nDynSdbAntd.SdDeleteAntd(): Entry");
            ...
            string delete_buf = String.Format(mDELETE, mTableName, acode, antang);

            //...Log2.v("\nDynSdbAntd.SdDeleteAntd(): query = " + delete_buf);
            ...
            if (mMdbWriteEnabled)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    string str = String.Format("sdDeleteAntd01 -- Error deleting from main.sd_antd");
                    Ssutil.DbGetDiagStmt(hStmt, str);
                    ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                    Log2.e("\nDynSdbAntd.SdDeleteAntd(): ERROR: SQLExecDirect() failed for query = " + delete_buf);
                    return Error.ODBC_EXECDIRECT_FAILED;
                }
            }
            else
            {
                Info.Text += "\r\n\r\n" + delete_buf;
                //...Log2.v("\nDynSdbAntd.SdDeleteAntd(): Deletion of main.sd_antd records is DISABLED.");
            }

            ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);

            //...Log2.v("\n\nDynSdbAntd.SdDeleteAntd(): Exit");
            return Constant.SUCCESS;
        }

        /// <summary>
        /// This method returns true if a record exists in the SDB table <b>main.sd_antd</b>
        /// that has a prescribed band code (bndcde).
        /// </summary>
        /// <param name="bndcde"> the prescribed band code.</param>
        /// <returns></returns>
