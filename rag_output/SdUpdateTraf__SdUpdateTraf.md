# Documented File: SdUpdateTraf.cs
**Repository Path:** `SdUpdateTraf\SdUpdateTraf.cs`
...
/// <summary>
/// This program updates the Subsidiary Database (SDB) table <b>main.sd_traf</b> using field
/// values provided in the prescribed user SDF table su_XXX_traf; 'traf' is an abbreviation 
/// for 'tower notes'. 
/// </summary>
/// <remarks>
/// The command-line usage is:
/// \image html "Usage - SdUpdateTraf.PNG" ""
/// </remarks>
namespace SdUpdateTraf
{
    ...
    /// <summary>
    /// This class provides the Main() method for the MICS program SdUpdateTraf.
    /// </summary>
    public class SdUpdateTraf
    {
        ...
        /// <summary>
        /// This is the Main() method for the MICS program SdUpdateTraf.
        /// </summary>
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                // user that executed this program (e.g. 'hulme'). Consequently, hulme.sd_traf
                // acts as a surrogate for main.sd_traf etc.
                ...
                {
                    Log2.e("\nSdUpdateTraf.Main(): ERROR: call to Qutils.EnterQueue() failed, rc = " + rc);
                ...
                DynSdbTraf.SetState(Info.MdbWriteEnabled, Info.SpoofModeIsOff, Info.GlobalSchema);
                ...
                BiUtil.BiBillingRec("UPD_TRAF", infoBuffer);
                ...
                Info.SetDateTimeNow();
                Console.Write("\t\tSDB TRAFFIC UPDATE REPORT Build {0}\t\tDate: {1} {2}\r\n",
                             Info.BuildMetaData, Info.Date, Info.Time);
                Console.Write("\r\n");
                Console.Write("SDF Name: {0}\r\n", Info.SdfName);

                // Check to see that the associated su_XXX_traf table for this
                // SDF base name (XXX) actually exists in the DB.
                if (!Ssutil.UtTableExist(Constant.SU_TRAF, Info.SdfName))
                {
                    Console.Write("\r\n************************************************\r\n");
                    Console.Write("\r\nTraf sdf {0} does not exist\r\n", Info.SdfName);
                    Console.Write("\r\n************************************************\r\n");
                ...
                if (rc != Constant.SUCCESS)
                {
                    ...
                    Console.Write("\r\nSdUpdateTraf: Could not set the database connection to manual commit.\r\n{0}\r\n",
                                    GenUtil.GetUserMess());
                    exitCode = 95;
                    goto cleanUpAndExit;
                }

                Console.Write("\r\n\t\tTRAFFIC SDF VALIDATION RESULTS\r\n");

                // Perform validation of the prescribed su_XXX_traf SDF file even if 
                ...
                // Check for failure of the validation of the prescribed su_XXX_traf SDF file.
                ...
                // This method provides the core functionality for SdUpdateTraf.
                // ============================================================
                rc = Updater.ProcessSuTraf(hConn, Info.SdfName,
                                                out errCount,
                                                out warnCount,
                                                out recCounts);
                ...
                AtRecordChng.AtRecordChange(Info.DbName, Info.SdfName, Constant.SU_TRAF);
            ...
            {
                Log2.e("\r\r\n\nSdUpdateTraf.Main(): exception caught: " + e.Message);
                Log2.e("\r\r\n\nSdUpdateTraf.Main(): stack trace: \r\r\n\n" + e.StackTrace);
                BiUtil.BiBillingRec(Constant.BI_END, "");
                Ssutil.UtDisconnect(1);
