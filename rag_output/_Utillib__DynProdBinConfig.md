# Documented File: DynProdBinConfig.cs
**Repository Path:** `_Utillib\DynProdBinConfig.cs`
...
namespace _Utillib
{
    ...
    /// <summary>
    /// This class provides methods to select, fetch, insert, update and delete equipment records from the table
    /// <b>hulme.prod_bin_config</b>.
    /// </summary>
    public class DynProdBinConfig
    {
        public const string mBaseTableName = "prod_bin_config";

        private const string mFullTableName = "hulme." + mBaseTableName;

        private static string mTableName = mFullTableName;

        private const string mSELECT = "SELECT * FROM {0} ";

        private const string mUPDATE = "UPDATE {0} SET {1} WHERE fileName='{2}' AND md5='{3}'  ";

        private const string mINSERT = "INSERT INTO {0} VALUES ({1}) ";

        private const string mDELETE = "DELETE FROM {0} WHERE fileName='{1}' AND md5='{2}'";
        ...
        private static int GetNextFreeCursor()
        {
            ...
            //...Log2.v("\r\nDynProdBinConfig.GetNextFreeCursor(): nNextFreeCursor = " + nNextFreeCursor);
            if (nNextFreeCursor >= Constant.NUM_CURSORS_FEW)
            {
                ...
                {
                    //...Log2.v("\r\nDynProdBinConfig.GetNextFreeCursor(): nInd = " + nInd);
                ...
                if (curHandle == -1)
                {
                    ...
                    GenUtil.SetErr("DynProdBinConfig01 -- No more open cursors.");
                    Application.Exit("\r\nDynProdBinConfig.GetNextFreeCursor(): ERROR: No available cursors.");
                    return -1;
                }
            ...
            //...Log2.v("\r\nDynProdBinConfig.GetNextFreeCursor(): found available cursor: curHandle = " + curHandle);
        ...
        /// <summary>
        /// Selects records from the database table <b>hulme.prod_bin_config</b> i.a.w.
        ...
        public static int Select(string searchCriteria, string orderBy)
        {
            ...
            if (mhStmt == SQLHANDLE.Zero)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    Log2.e("\nDynProdBinConfig.Select(): ERROR : Exit: call to SQLAllocHandle() failed, sqlRet = " + sqlRet);
                    return Error.ODBC_SQLALLOCHANDLE_FAILED;
                }

                ProdBinConfig.BindPtrsToCols(mhStmt, out mTgtValPtrs, out mNullIndPtrs);
            }
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\nDynProdBinConfig.Select(): ERROR : Exit: Attempt to close the cursor on the reusable statement handle failed.");
            ...
            //...Log2.v("\r\nDynProdBinConfig.Select(): stmt_buf = \r\n" + stmt_buf);

            sqlRet = ODBC.SQLExecDirect(mhStmt, stmt_buf, stmt_buf.Length);

            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\nDynProdBinConfig.Select(): ERROR : Exit: call to SQLExecDirect() failed, sqlRet = " + sqlRet);
            ...
            //...Log2.v("\n\nDynProdBinConfig.Select(): Exit");
            return (curHandle);
        }

        /// <summary>
        /// Retrieves a single equipment record from the database table <b>hulme.prod_bin_config</b> using 
        /// the cursor object created by a previous call to Select().
        /// </summary>
        /// <param name="nCursor"> - FtCursor object.</param>
        /// <param name="prodBinConfig"> - a ProdBinConfig object populated with data from the row.</param>
        /// <param name="nullInds"> - array of ODBC nullInds for prodBinConfig.</param>
        /// <returns></returns>
        ...
        public static int Fetch(int nCursor, out ProdBinConfig prodBinConfig, out SQLLEN[] nullInds)
        {
            // 'out' requirments.
            prodBinConfig = null;
            nullInds = null;

            //...Log2.v("\n\nDynProdBinConfig.Fetch(): Entry");
            ...
            {
                Log2.e("\n\nDynProdBinConfig.Fetch(): Exit: ERROR: failed contiguity test.");

                return -666;
            }

            // Fetch the next row of selected data.
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                ...
                {
                    //...Log2.v("\n\nDynProdBinConfig.Fetch(): Exit: sqlRet == ODBC.SQL_NO_DATA");
                    return ODBC.SQL_NO_DATA;
                }
                else
                {
                    // We have an error.
                    Log2.e("\n\nDynProdBinConfig.Fetch(): Exit: ERROR: SQLFetch(): sqlRet = " + sqlRet);
            ...
            else
            {
                ...
                try
                {
                    ProdBinConfig.ReadColBindings(mTgtValPtrs, mNullIndPtrs, out prodBinConfig, out nullInds);
                ...
                catch (Exception e)
                {
                    ...
                    Log2.e("\n\nDynProdBinConfig.Fetch(): Exit: ERROR: call to ReadColBindings() threw an exception:\n" + e.Message);
                    return -22;
                } //try-catch

            } //if-else

            //...Log2.v(prodBinConfig.ToStringWN(nullInds));

            //...Log2.v("\n\nDynProdBinConfig.Fetch(): Exit");
            return nRet;
        } //end of Fetch
        ...
        /// <summary>
        /// This method updates all of the column values of a prescribed record in the table
        /// hulme.prod_bin_config.
        /// </summary>
        /// <param name="hConn"> - an open ODBC connection handle.</param>
        /// <param name="prodBinConfig"> - a prescribed ProdBinConfig object that provides the updated field values.</param>
        /// <param name="nullInds"> - an array of ODBC nullInds associated with prodBinConfig.</param>
        /// <returns></returns>
        public static int Update(SQLHANDLE hConn, ProdBinConfig prodBinConfig, SQLLEN[] nullInds)
        {
            //...Log2.v("\nDynProdBinConfig.Update(): Entry");
            ...
            update_buf = String.Format(mUPDATE, mTableName, prodBinConfig.ToStringAsCSVequates(nullInds), prodBinConfig.FileName, prodBinConfig.MD5hash);

            //...Log2.v("\nDynProdBinConfig.Update(): query = " + update_buf);

            sqlRet = ODBC.SQLExecDirect(hStmt, update_buf, update_buf.Length);

            if (!ODBC.IsOK(sqlRet))
            {
                string str = String.Format("\nDynProdBinConfig.Update(): ERROR: SQLExecDirect failed, sqlRet = {0}, SQL query:\n{1}", sqlRet, update_buf);
                Log2.e(str);
                str = String.Format("DynProdBinConfig.Update() 04 -- Error writing prodBinConfig: {0} {1}", prodBinConfig.FileName, prodBinConfig.MD5hash);
            ...
            //...Log2.v("\nDynProdBinConfig.Update(): Exit");
            return (Constant.SUCCESS);
        }

        /// <summary>
        /// This method inserts a record into the table hulme.prod_bin_config using column values prescribed 
        /// by the fields of a prescribed ProdBinConfig object.
        /// </summary>
        /// <param name="hConn"> - an open ODBC connection handle.</param>
        /// <param name="prodBinConfig"> - a ProdBinConfig object.</param>
        /// <param name="nullInd"> - array of ODBC nullInds for prodBinConfig</param>
        /// <returns></returns>
        ...
        public static int Insert(SQLHANDLE hConn, ProdBinConfig prodBinConfig, SQLLEN[] nullInd)
        {
            //...Log2.v("\n\nDynProdBinConfig.Insert(): Entry");
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\n\nDynProdBinConfig.Insert(): ERROR: call to SQLAllocHandle() failed.");
                return Error.ODBC_SQLALLOCHANDLE_FAILED;
            }

            //Create the SQL insert statement.
            cSQL = String.Format(mINSERT, mTableName, prodBinConfig.ToStringAsQuotedCSV());

            //...Log2.v("nDynProdBinConfig.Insert(): SQLExecDirect():\r\n" + cSQL);

            sqlRet = ODBC.SQLExecDirect(hStmt, cSQL, cSQL.Length);

            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\n\nDynProdBinConfig.Insert(): ERROR: a call to SQLExecDirect() failed for: " + cSQL);
                string str = "Insert ProdBinConfig 02 -- Error inserting record";
                Ssutil.DbGetDiagStmt(hStmt, str); ;
                ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                return Error.ODBC_EXECDIRECT_FAILED;
            }

            //...Log2.v("\n\nDynProdBinConfig.Insert(): successfuly inserted record into table hulme.prod_bin_config");

            //This method call releases hStmt, disconnects from the DB and then releases hConn.
            ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);

            //...Log2.v("\n\nDynProdBinConfig.Insert(): Exit");
            return Constant.SUCCESS;
        }

        /// <summary>
        /// This method deletes a record with prescribed keys (fileName, md5) from the
        /// table hulme.prod_bin_config.
        /// </summary>
        /// <param name="hConn"> - an open ODBC connection handle.</param>
        /// <param name="fileName"></param>
        /// <param name="md5"></param>
        /// <returns></returns>
        public static int Delete(SQLHANDLE hConn, string fileName, string md5)
        {
            //...Log2.v("\n\nDynProdBinConfig.Delete(): Entry");
            ...
            string delete_buf = String.Format(mDELETE, mTableName, fileName, md5);

            //...Log2.v("\nDynProdBinConfig.Delete(): query = " + delete_buf);

            sqlRet = ODBC.SQLExecDirect(hStmt, delete_buf, delete_buf.Length);

            if (!ODBC.IsOK(sqlRet))
            {
                string str = String.Format("Delete ProdBinConfig 01 -- Error deleting from hulme.prod_bin_config");
                Ssutil.DbGetDiagStmt(hStmt, str);
                ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                Log2.e("\nDynProdBinConfig.Delete(): ERROR: SQLExecDirect() failed for query = " + delete_buf);
                return Error.ODBC_EXECDIRECT_FAILED;
            }

            ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);

            //...Log2.v("\n\nDynProdBinConfig.Delete(): Exit");
            return 0;
        }

        /// <summary>
        /// This method returns true if a record exists in the table <b>hulme.prod_bin_config</b>
        /// that has a prescribed key (fileName, md5).
        /// </summary>
        /// <param name="fileName"></param>
        /// <param name="md5"></param>
        /// <returns></returns>
        public static bool RecordExistsWithKey(string fileName, string md5)
        {
            string whereClause = String.Format(" fileName = '{0}' AND md5 = '{1}'", fileName, md5);
        ...
        /// <summary>
        /// This method returns a populated ProdBinConfig object and associated ODBC nullInds
        /// corresponding to the record in the table 'hulme.prod_bin_config' that has the 
        /// prescribed key (fileName, md5).
        /// </summary>
        /// <param name="fileName"></param>
        /// <param name="md5"></param>
        ...
        public static bool FetchRecordWithKey(string fileName, string md5, out ProdBinConfig prodBinConfig, out SQLLEN[] nullInds)
        {
            // 'out' requirements.
            prodBinConfig = null;
            nullInds = null;
            ...
            if (nCursor < 0)
            {
                Log2.e("\nDynProdBinConfig.FetchRecordWithKeys(): ERROR: Select() failed, nCursor = " + nCursor);
                return false;
            }

            // Do the 'FETCH'.
            int rc = Fetch(nCursor, out prodBinConfig, out nullInds);
            ...
            {
                Log2.e("\nDynProdBinConfig.FetchRecordWithKeys(): ERROR: Fetch() failed, rc = " + rc);
                isSuccessful = false;
            }
