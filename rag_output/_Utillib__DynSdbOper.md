# Documented File: DynSdbOper.cs
**Repository Path:** `_Utillib\DynSdbOper.cs`
...
namespace _Utillib
{
    ...
    /// <summary>
    /// This class provides methods to select, fetch, insert, update and delete operator records from the subsiduary database table
    /// <b>main.sd_note</b>.
    /// </summary>
    public class DynSdbOper
    {
        private const string mBaseTableName = "sd_oper";
        ...
        private const string mUPDATE = "UPDATE {0} SET {1} WHERE oper='{2}' ";

        private const string mINSERT = "INSERT INTO {0} VALUES ({1}) ";

        private const string mDELETE = "DELETE FROM {0} WHERE oper='{1}' ";
        ...
        /// <summary>
        /// This method can be used to set whether the SDB <b>main.sd_oper</b> table should be written to,
        /// (SQL DELETE, INSERT and/or UPDATE queries) or not; it can also set whether the SQL queries should
        /// target the actual main.sd_oper table or a user's 'spoof' table (e.g. hulme.sd_oper).
        /// </summary>
        /// <param name="mdbWriteEnabled"> - boolean that prescribes whether sd_oper record
        ...
        public static void SetState(bool mdbWriteEnabled, bool spoofModeIsOff, string userSchema)
        {
            ...
            else
            {
                ...
                {
                    Log2.e("\nDynSdbOper.SetState(): ERROR: spoofMode is ON but userSchema is null or whitespace.");
        ...
        private static int GetNextFreeCursor()
        {
            ...
            //...Log2.v("\r\nDynSdbOper.GetNextFreeCursor(): nNextFreeCursor = " + nNextFreeCursor);
            if (nNextFreeCursor >= Constant.NUM_CURSORS_FEW)
            {
                ...
                {
                    //...Log2.v("\r\nDynSdbOper.GetNextFreeCursor(): nInd = " + nInd);
                ...
                if (curHandle == -1)
                {
                    ...
                    GenUtil.SetErr("DynSdbOper01 -- No more open cursors.");
                    Application.Exit("\r\nDynSdbOper.GetNextFreeCursor(): ERROR: No available cursors.");
                    return -1;
                }
            ...
            //...Log2.v("\r\nDynSdbOper.GetNextFreeCursor(): found available cursor: curHandle = " + curHandle);
        ...
        /// <summary>
        /// Selects records from the database table <b>main.sd_oper</b> i.a.w.
        ...
        /// <returns></returns>
        /// <para>- non-negative value - the index of the cursor to be used for SdFetchOper() calls.</para>
        /// <para>- ErrorMessages.NO_CURSOR_AVAILABLE - reached limit for the number of cursors that can be open concurrently.</para>
        /// <para>-   ErrorMessages.ODBC_EXECUTE_FAILED    - call to ODBC.SQLExecute() failed to return data. </para>
        public static int SdSelectOper(string searchCriteria, string orderBy)
        {
            //...Log2.v("\n\nDynSdbOper.SdSelectOper(): Entry");
            ...
            if (mhStmt == SQLHANDLE.Zero)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    Log2.e("\nDynSdbOper.SdSelectOper(): ERROR : Exit: call to SQLAllocHandle() failed, sqlRet = " + sqlRet);
                    return Error.ODBC_SQLALLOCHANDLE_FAILED;
                }

                SdOper.BindPtrsToCols(mhStmt, out mTgtValPtrs, out mNullIndPtrs);
            }
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\nDynSdbOper.SdSelectOper(): ERROR : Exit: Attempt to close the cursor on the reusable statement handle failed.");
            ...
            //...Log2.v("\r\nDynSdbOper.SdSelectOper(): stmt_buf = \r\n" + stmt_buf);

            sqlRet = ODBC.SQLExecDirect(mhStmt, stmt_buf, stmt_buf.Length);

            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\nDynSdbOper.SdSelectOper(): ERROR : Exit: call to SQLExecDirect() failed, sqlRet = " + sqlRet);
            ...
            //...Log2.v("\n\nDynSdbOper.SdSelectOper(): Exit");
            return (curHandle);
        }

        /// <summary>
        /// Retrieves a single operator record from the database table <b>main.sd_oper</b> using 
        /// the cursor object created by a previous call to SdSelectOper().
        /// </summary>
        /// <param name="nCursor"> - FtCursor object.</param>
        /// <param name="sdOper"> - a SdOper object populated with data from the row.</param>
        /// <param name="nullInds"> - array of ODBC nullInds for sdOper.</param>
        /// <returns></returns>
        ...
        public static int SdFetchOper(int nCursor, out SdOper sdOper, out SQLLEN[] nullInds)
        {
            // 'out' requirments.
            sdOper = null;
            nullInds = null;

            //...Log2.v("\n\nDynSdbOper.SdFetchOper(): Entry");
            ...
            {
                Log2.e("\n\nDynSdbOper.SdFetchOper(): Exit: ERROR: failed contiguity test.");

                return -666;
            }

            // Fetch the next row of selected Oper data.
            SQLRETURN sqlRet = ODBC.SQLFetch(mhStmt);

            // Check the SQLRETURN and act accordingly.
            if (!ODBC.IsOK(sqlRet))
            {
                // If no more Oper records available then break out the while-loop.
                if (sqlRet == ODBC.SQL_NO_DATA)
                {
                    //...Log2.v("\n\nDynSdbOper.SdFetchOper(): Exit: sqlRet == ODBC.SQL_NO_DATA");
                    return ODBC.SQL_NO_DATA;
                }
                else
                {
                    // We have an error.
                    Log2.e("\n\nDynSdbOper.SdFetchOper(): Exit: ERROR: SQLFetch(): sqlRet = " + sqlRet);
            ...
            else
            {
                ...
                try
                {
                    SdOper.ReadColBindings(mTgtValPtrs, mNullIndPtrs, out sdOper, out nullInds);
                ...
                catch (Exception e)
                {
                    ...
                    Log2.e("\n\nDynSdbOper.SdFetchOper(): Exit: ERROR: call to ReadColBindings() threw an exception:\n" + e.Message);
                    return -22;
                } //try-catch

            } //if-else

            //...Log2.v(sdOper.ToStringWN(nullInds));

            //...Log2.v("\n\nDynSdbOper.SdFetchOper(): Exit");
            return nRet;
        } //end of SdFetchOper

        /// <summary>
        /// Closes (releases) a cursor instantiated by a previous call to SdSelectOper(); 
        ...
        /// <para>-   ErrorMessages.DYN_CUR_NOT_OPEN  - FtCursor field cursorOpen is set to false.</para>
        public static int SdCloseOper(int curHandle)
        {
        ...
        /// <summary>
        /// This method updates all of the column values of a prescribed record in the SDB Oper table
        /// main.sd_oper.
        /// </summary>
        /// <param name="hConn"> - an open ODBC connection handle.</param>
        /// <param name="sdOper"> - a prescribed SdOper object that provides the updated field values.</param>
        /// <param name="nullInds"> - an array of ODBC nullInds associated with sdOper.</param>
        /// <returns></returns>
        public static int SdUpdateOper(SQLHANDLE hConn, SdOper sdOper, SQLLEN[] nullInds)
        {
            //...Log2.v("\nDynSdbOper.SdUpdateOper(): Entry");
            ...
            update_buf = String.Format(mUPDATE, mTableName, sdOper.ToStringAsCSVequates(nullInds), sdOper.oper);

            //...Log2.v("\nDynSdbOper.SdUpdateOper(): query = " + update_buf);
            ...
            if (mMdbWriteEnabled)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    string str = String.Format("\nDynSdbOper.SdUpdateOper(): ERROR: SQLExecDirect failed, sqlRet = {0}, SQL query:\n{1}", sqlRet, update_buf);
                    Log2.e(str);
                    str = String.Format("mtUpdateOper04 -- Error writing Oper: {0}", sdOper.oper);
            ...
            {
                Info.Text += "\r\n\r\n" + update_buf;
                //...Log2.v("\nDynSdbOper.SdUpdateOper(): Updating of main.sd_oper records is DISABLED.");
            }

            ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);

            //...Log2.v("\nDynSdbOper.SdUpdateOper(): Exit");
            return (Constant.SUCCESS);
        }

        /// <summary>
        /// This method inserts a record into the SDB Oper table main.sd_oper using column values prescribed 
        /// by the fields of a prescribed SdOper object.
        /// </summary>
        /// <param name="hConn"> - an open ODBC connection handle.</param>
        /// <param name="sdOper"> - a SdOper object.</param>
        /// <param name="nullInd"> - array of ODBC nullInds for sdOper</param>
        /// <returns></returns>
        ...
        public static int SdInsertOper(SQLHANDLE hConn, SdOper sdOper, SQLLEN[] nullInd)
        {
            //...Log2.v("\n\nDynSdbOper.SdInsertOper(): Entry");
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\n\nDynSdbOper.SdInsertOper(): ERROR: call to SQLAllocHandle() failed.");
                return Error.ODBC_SQLALLOCHANDLE_FAILED;
            }

            //Create the SQL insert statement.
            cSQL = String.Format(mINSERT, mTableName, sdOper.ToStringAsCSV(nullInd));

            //...Log2.v("nDynSdOper.SdInsertOper(): SQLExecDirect():\r\n" + cSQL);
            ...
            if (mMdbWriteEnabled)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    Log2.e("\n\nDynSdbOper.SdInsertOper(): ERROR: a call to SQLExecDirect() failed for: " + cSQL);
                    string str = "mtInsertOper02 -- Error inserting operator";
                    Ssutil.DbGetDiagStmt(hStmt, str); ;
                    ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                    return Error.ODBC_EXECDIRECT_FAILED;
                }

                //...Log2.v("\n\nDynSdbOper.SdInsertOper(): successfuly inserted record into table main.sd_oper");
            }
            else
            {
                Info.Text += "\r\n\r\n" + cSQL;
                //...Log2.v("\nDynSdbOper.SdInsertOper(): Insertion of main.sd_oper records is DISABLED.");
            }

            //This method call releases hStmt, disconnects from the DB and then releases hConn.
            ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);

            //...Log2.v("\n\nDynSdbOper.SdInsertOper(): Exit");
            return Constant.SUCCESS;
        }

        /// <summary>
        /// This method deletes a record with prescribed key (oper) from the SDB
        /// Oper table main.sd_oper.
        /// </summary>
        /// <param name="hConn"> - an open ODBC connection handle.</param>
        /// <param name="oper"></param>
        /// <returns></returns>
        public static int SdDeleteOper(SQLHANDLE hConn, string oper)
        {
            //...Log2.v("\n\nDynSdbOper.SdDeleteOper(): Entry");
            ...
            string delete_buf = String.Format(mDELETE, mTableName, oper);

            //...Log2.v("\nDynSdbOper.SdDeleteOper(): query = " + delete_buf);
            ...
            if (mMdbWriteEnabled)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    string str = String.Format("mtDeleteOper01 -- Error deleting from main.sd_oper");
                    Ssutil.DbGetDiagStmt(hStmt, str);
                    ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                    Log2.e("\nDynSdbOper.SdDeleteOper(): ERROR: SQLExecDirect() failed for query = " + delete_buf);
                    return Error.ODBC_EXECDIRECT_FAILED;
                }
            }
            else
            {
                Info.Text += "\r\n\r\n" + delete_buf;
                //...Log2.v("\nDynSdbOper.SdDeleteOper(): Deletion of main.sd_oper records is DISABLED.");
            }

            ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);

            //...Log2.v("\n\nDynSdbOper.SdDeleteOper(): Exit");
            return 0;
        }

        /// <summary>
        /// This method returns true if a record exists in the SDB Oper table <b>main.sd_oper</b>
        /// that has a prescribed key (oper).
        /// </summary>
        /// <param name="oper"></param>
        /// <returns></returns>
        public static bool SdbRecordExistsWithKey(string oper)
        {
            string whereClause = String.Format(" oper = '{0}'", oper);
        ...
        /// <summary>
        /// This method returns a populated SdOper object and associated ODBC nullInds
        /// corresponding to the record in the main SDB Oper table 'main.sd_oper' that has the 
        /// prescribed key (oper).
        /// </summary>
        /// <param name="oper"></param>
        ...
        public static bool FetchRecordWithKey(string oper, out SdOper sdOper, out SQLLEN[] nullInds)
        {
            // 'out' requirements.
            sdOper = null;
            nullInds = null;
            ...
            // Do the 'SELECT'.
            int nCursor = SdSelectOper(whereClause, "");

            if (nCursor < 0)
            {
                Log2.e("\nDynSdbOper.FetchRecordWithKeys(): ERROR: Select() failed, nCursor = " + nCursor);
                return false;
            }

            // Do the 'FETCH'.
            int rc = SdFetchOper(nCursor, out sdOper, out nullInds);
            ...
            {
                Log2.e("\nDynSdbOper.FetchRecordWithKeys(): ERROR: Fetch() failed, rc = " + rc);
                isSuccessful = false;
            }
