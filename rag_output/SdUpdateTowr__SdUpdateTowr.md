# Documented File: SdUpdateTowr.cs
**Repository Path:** `SdUpdateTowr\SdUpdateTowr.cs`
...
/// <summary>
/// This program updates the Subsidiary Database (SDB) table <b>main.sd_towr</b> using field
/// values provided in the prescribed user SDF table su_XXX_towr; 'towr' is an abbreviation 
/// for 'tower'. 
/// </summary>
/// <remarks>
/// The command-line usage is:
/// \image html "Usage - SdUpdateTowr.PNG" ""
/// </remarks>
namespace SdUpdateTowr
{
    ...
    /// <summary>
    /// This class provides the Main() method for the MICS program SdUpdateTowr.
    /// </summary>
    public class SdUpdateTowr
    {
        ...
        /// <summary>
        /// This is the Main() method for the MICS program SdUpdateTowr.
        /// </summary>
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                // user that executed this program (e.g. 'hulme'). Consequently, hulme.sd_towr
                // acts as a surrogate for main.sd_towr etc.
                ...
                {
                    Log2.e("\nSdUpdateTowr.Main(): ERROR: call to Qutils.EnterQueue() failed, rc = " + rc);
                ...
                DynSdbTowr.SetState(Info.MdbWriteEnabled, Info.SpoofModeIsOff, Info.GlobalSchema);
                ...
                BiUtil.BiBillingRec("UPD_TOWR", infoBuffer);

                // Initialize the processing status for this SDF to OK.
                int status = Constant.OK;

                // Write the build information for this version of SdUpdateTowr.
                Console.Write("\r\nsdUpdateTowr Build {0}\r\n", Info.BuildMetaData);

                // Get the current date and time from the system and print it
                // at the top of each new report or validation.
                Info.SetDateTimeNow();
                Console.Write("\t\tSDB TOWER UPDATE REPORT Build {0}\t\tDate: {1} {2}\r\n",
                             Info.BuildMetaData, Info.Date, Info.Time);
                Console.Write("\r\n");
                Console.Write("SDF Name: {0}\r\n", Info.SdfName);
                Console.Write("SDF Type: Tower\r\n");

                // Check to see that the associated su_XXX_towr table for this
                // SDF base name (XXX) actually exists in the DB.
                if (!Ssutil.UtTableExist(Constant.SU_TOWR, Info.SdfName))
                {
                    Console.Write("\r\n************************************************\r\n");
                    Console.Write("\r\nTowr sdf {0} does not exist\r\n", Info.SdfName);
                    Console.Write("\r\n************************************************\r\n");
                ...
                if (rc != Constant.SUCCESS)
                {
                    ...
                    Console.Write("\r\nSdUpdateTowr: Could not set the database connection to manual commit.\r\n{0}\r\n",
                                    GenUtil.GetUserMess());
                    exitCode = 95;
                    goto cleanUpAndExit;
                }
                ...
                // Perform validation of the prescribed su_XXX_towr SDF file even if 
                ...
                // Check for failure of the validation of the prescribed su_XXX_towr SDF file.
                ...
                // This method provides the core functionality for SdUpdateTowr.
                // ============================================================
                rc = Updater.ProcessSuTowr(hConn, Info.SdfName,
                                                out errCount,
                                                out warnCount,
                                                out recCounts);
                ...
                AtRecordChng.AtRecordChange(Info.DbName, Info.SdfName, Constant.SU_TOWR);
            ...
            {
                Log2.e("\r\r\n\nSdUpdateTowr.Main(): exception caught: " + e.Message);
                Log2.e("\r\r\n\nSdUpdateTowr.Main(): stack trace: \r\r\n\n" + e.StackTrace);
                BiUtil.BiBillingRec(Constant.BI_END, "");
                Ssutil.UtDisconnect(1);
        ...
        {
            Console.Write("\r\n");
            Console.Write("\r\n This program updates the Subsidiary Database (SDB) table main.sd_towr using field   +");
            Console.Write("\r\n values provided in the prescribed user SDF table [user].su_[sdfName]_towr.");
            Console.Write("\r\n");
            Console.Write("\r\n USAGE: SdUpdateTowr <dbName> <projCode> <sdfName> [-c] [-d] [-p] [-s] [-t]");
            Console.Write("\r\n ===== ");
            Console.Write("\r\n");
            Console.Write("\r\n        dbName           : database name, e.g. 'fcsa'.");
            Console.Write("\r\n        project          : user's project 'charge' code.");
            Console.Write("\r\n        sdfName          : the XXX in user table su_XXX_towr.");
            Console.Write("\r\n");
            Console.Write("\r\n        --- For Testing -------------------------------------------------------------");
            ...
            Console.Write("\r\n");
            Console.Write("\r\n e.g.");
            Console.Write("\r\n     SdUpdateTowr fcsa HULME1_0 test_combo0");
            Console.Write("\r\n");
            Console.Write("\r\n Notes:");
        ...
        {
            Console.Write("\r\n\r\n\t************ Summary Information for SDB Tower Update ****\r\n");
