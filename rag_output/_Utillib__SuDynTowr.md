# Documented File: SuDynTowr.cs
**Repository Path:** `_Utillib\SuDynTowr.cs`
...
namespace _Utillib
{
    ...
    /// <summary>
    /// This class provides methods that serve SuTowr objects to/from a database.
    /// </summary>
    public class SuDynTowr
    {
        //===================================================================================================================================
        ...
        public static int GetNextFreeCursor()
        {
            ...
            if (nNextFreeCursor >= Constant.NUM_CURSORS_FEW)
            {
                ...
                if (curHandle == -1)
                {
                    Application.Exit("\r\nSuDynTowr.GetNextFreeCursor(): ERROR: No available cursors.");
                }
            }
            else
            {
        ...
        /// <summary>
        /// Selects SuTowr records from a user's su_XXX_towr DB table for
        ...
        /// <returns></returns>
        /// <para>- non-negative value - the index of the cursor to be used for SuFetchTowr() calls.</para>
        /// <para>- ErrorMessages.NO_CURSOR_AVAILABLE - reached limit for the number of cursors that can be open concurrently.</para>
        /// <para>-   ErrorMessages.ODBC_EXECDIRECT_FAILED    - call to ODBC.SQLExecute() failed to return data. </para>
        public static int SuSelectTowr(string table, string searchCriteria, string orderBy)
        {
            //...Log2.v("\n\nSuDynTowr.SuSelectTowr(): Entry");
            SQLHANDLE hStmt;
            SQLHANDLE hUpdate;
            SQLRETURN sqlRet;
            ...
            string stmt_buf = "SELECT " + SuTowr.AllColumnsForSqlSelect + " FROM " + table;
            ...
            //...Log2.v("\r\nSuDynTowr.SuSelectTowr(): [1] sqlRet = " + sqlRet);

            //...Log2.v("\nSuDynTowr.SuSelectTowr(): query = " + stmt_buf);

            sqlRet = ODBC.SQLExecDirect(hStmt, stmt_buf, ODBC.SQL_NTS);

            //...Log2.v("\r\nSuDynTowr.SuSelectTowr(): [4] sqlRet = " + sqlRet);

            if (!ODBC.IsOK(sqlRet))
            {
                Ssutil.DbGetDiagStmt(hStmt, "Could not Select Towr for criteria: " + searchCriteria + ".");
                ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                Ssutil.DisConn(hConn);
                Log2.e("\nSuDynTowr.SuSelectTowr(): ERROR: SQLExecute() failed");
            ...
            //...Log2.v("\n\nSuDynTowr.SuSelectTowr(): Exit");
            return curHandle;
        }

        /// <summary>
        /// Retrieves a single row of SuTowr data from a table in the database using 
        /// a Cursor object previously created by a call to SuSelectTowr().
        /// </summary>
        /// <param name="curHandle"> - index of a Cursor object.</param>
        /// <param name="suTowr"> - a SuTowr object populated with data from the row.</param>
        /// <param name="nullInd"> - array of ODBC nullInds for suTowr.</param>
        /// <returns></returns>
        ...
        public static short SuFetchTowr(int curHandle, out SuTowr suTowr, out SQLLEN[] nullInd)
        {
            //...Log2.v("\n\nSuDynTowr.SuFetchTowr(): Entry:");

            //Create an SuTowr object to output.
            suTowr = new SuTowr();
            //Create an array of nulls to output.
            nullInd = new SQLLEN[SuTowr.NUM_COLUMNS];
            ...
            if (!cursors[curHandle].cursorOpen)
            {
                ...
                Log2.e("\n\nSuDynTowr.SuFetchTowr(): Exit: FAIL: if (!cursors[curHandle].cursorOpen)");
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                ...
                {
                    //...Log2.v("\n\nSuDynTowr.SuFetchTowr(): Exit: ODBC.SQLFetch(): sqlRet == ODBC.SQL_NO_DATA");
                    return sqlRet;
                }
                else
                {
                    //	Error
                    Log2.e("\n\nSuDynTowr.SuFetchTowr(): Exit: ODBC.SQLFetch(): FAIL: sqlRet = " + sqlRet);
            ...
            try
            {
                Ssutil.DbGetString(hStmt, 1, "cmd", out suTowr.cmd, SuTowr.CMD_SZ, out nullInd[SuTowr.CMD]);
                Ssutil.DbGetString(hStmt, 2, "recstat", out suTowr.recstat, SuTowr.RECSTAT_SZ, out nullInd[SuTowr.RECSTAT]);
                Ssutil.DbGetString(hStmt, 3, "twcode", out suTowr.twcode, SuTowr.TWCODE_SZ, out nullInd[SuTowr.TWCODE]);
                Ssutil.DbGetString(hStmt, 4, "twdesc", out suTowr.twdesc, SuTowr.TWDESC_SZ, out nullInd[SuTowr.TWDESC]);
                Ssutil.DbGetString(hStmt, 5, "mdate", out suTowr.mdate, SuTowr.MDATE_SZ, out nullInd[SuTowr.MDATE]);
                Ssutil.DbGetString(hStmt, 6, "mtime", out suTowr.mtime, SuTowr.MTIME_SZ, out nullInd[SuTowr.MTIME]);
            }
            catch (Exception e)
            {
                Log2.e("\n\nSuDynTowr.SuFetchTowr(): ERROR: Exception caught: " + e.Message);
                GenUtil.SetErr("dynfeTowr02 -- Could not get band, because of field " + e.Message);
                return Error.ODBC_GET_FAILED;
            }

            //...Log2.v("\n\nSuDynTowr.SuFetchTowr(): Exit");
        ...
        /// <summary>
        /// Closes (releases) a cursor instantiated by a previous call to SuSelectTowr(); 
        ...
        public static int SuCloseTowr(int curHandle)
        {
