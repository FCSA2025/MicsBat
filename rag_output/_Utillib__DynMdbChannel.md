# Documented File: DynMdbChannel.cs
**Repository Path:** `_Utillib\DynMdbChannel.cs`
...
namespace _Utillib
{
    ...
    {
        private const string mBaseTableName = "mt_chan";
        ...
        private const string mUPDATE = "UPDATE {0} SET {1} WHERE call1='{2}' AND call2='{3}' AND bndcde='{4}' AND chid='{5}' ";

        private const string mINSERT = "INSERT INTO {0} VALUES ({1}) ";

        private const string mDELETE = "DELETE FROM {0} WHERE call1='{1}' AND call2='{2}' AND bndcde='{3}' AND chid='{4}' ";

        private const string mCHNG_UPDATE_1 = "UPDATE {0} SET call1='{1}', userid='{2}', mdate='{3}', mtime='{4}' WHERE call1='{5}' ";

        private const string mCHNG_UPDATE_2 = "UPDATE {0} SET call2='{1}', userid='{2}', mdate='{3}', mtime='{4}' WHERE call2='{5}' ";

        public enum NODE { LOCAL, REMOTE }
        ...
        // We need to remember the magic number for every handle ID.
        private static int[] mHandleMagicNumbers = new int[5] { 0, 0, 0, 0, 0 };
        //=================================================================================================================
        ...
        /// <summary>
        /// This method can be used to set whether the MDB <b>main.mt_chan</b> table should be written to,
        /// (SQL DELETE, INSERT and/or UPDATE queries) or not; it can also set whether the SQL queries should
        /// target the actual main.mt_chan table or a user's 'spoof' table (e.g. hulme.audit_trail).
        /// </summary>
        /// <param name="mdbWriteEnabled"> - boolean that prescribes whether mt_chan record
        ...
        public static void SetState(bool mdbWriteEnabled, bool spoofModeIsOff, string userSchema)
        {
            ...
            else
            {
                ...
                {
                    Log2.e("\nDynMdbChannel.SetState(): ERROR: spoofMode is ON but userSchema is null or whitespace.");
        ...
        public static int MtSelectChannel(string searchCriteria, string orderBy)
        {
            ...
            if (mhStmt == SQLHANDLE.Zero)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    Log2.e("\nDynMdbChannel.MtSelectChannel(): ERROR : Exit: call to SQLAllocHandle() failed, sqlRet = " + sqlRet);
                    return Error.ODBC_SQLALLOCHANDLE_FAILED;
                }

                MtChan.BindPtrsToCols(mhStmt, out mTgtValPtrs, out mNullIndPtrs);
            }
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\nDynMdbChannel.MtSelectChannel(): ERROR : Exit: Attempt to close the cursor on the reusable statement handle failed.");
                Application.Exit(666);
            }

            // Create the SQL query string.
            string cSQL = String.Format(mSELECT, mTableName);
            ...
            sqlRet = ODBC.SQLExecDirect(mhStmt, cSQL, cSQL.Length);

            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\nDynMdbChannel.MtSelectChannel(): ERROR : Exit: call to SQLExecDirect() failed, cSQL = " + cSQL);
            ...
            mHandleMagicNumbers[curHandle] = mMagicNumber;

            //...Log2.v("\n\nDynMdbChannel.MtSelectChannel(): Exit: " + String.Format("curHandle = {0}, cursors[curHandle].magicNumber = {1}", curHandle, cursors[curHandle].magicNumber));

            return (curHandle);
        }

        /// <summary>
        /// Retrieves a single row of channel data from the database table <b>main.mt_chan</b> using 
        ...
        /// <param name="nullInds"> - array of ODBC nullInds for mtChan.</param>
        /// <returns></returns>
        ...
        public static int MtFetchChannel(int nCursor, out MtChan mtChan, out SQLLEN[] nullInds)
        {
            // 'out' requirments.
            mtChan = null;
            nullInds = null;
            ...
            if (cursors[nCursor].magicNumber != mHandleMagicNumbers[nCursor])
            {
                Log2.e("\n\nDynMdbChannel.MtFetchChannel(): Exit: ERROR: failed contiguity test: " + String.Format("nCursor = {0}, cursors[nCursor].magicNumber = {1}, mHandleMagicNumbers[nCursor] = {2}", nCursor, cursors[nCursor].magicNumber, mHandleMagicNumbers[nCursor]));
                return -666;
            }

            // Fetch the next row of selected Channel data.
            ...
            else
            {
                ...
                try
                {
                    MtChan.ReadColBindings(mTgtValPtrs, mNullIndPtrs, out mtChan, out nullInds);
                ...
                catch (Exception e)
                {
                    ...
                    Log2.e("\n\nDynMdbChannel.MtFetchChannel(): Exit: ERROR: call to ReadColBindings() threw an exception:\n" + e.Message);
                    return -22;
                } //try-catch

            } //if-else

            //...Log2.v(mtChan.ToStringWN(nullInds));
        ...
        {
            //...Log2.v("\n\nDynMdbChannel.MtCloseChannel(): Entry: curHandle = " + curHandle);
            ...
            //...Log2.v("\n\nDynMdbChannel.MtCloseChannel(): Exit");
            return 0;
        }

        /// <summary>
        /// This method returns true if a record exists in the MDB table <b>main.mt_chan</b>
        /// that has a prescribed key: call1, call2, bndcde and chid.
        /// </summary>
        ...
        /// <returns></returns>
        public static bool MdbRecordExists(string call1, string call2, string bndcde, string chid)
        {
            string whereClause = String.Format(" call1 = '{0}' AND call2 = '{1}' AND bndcde = '{2}' AND chid = '{3}' ",
                                                call1, call2, bndcde, chid);

            // These for columns comprise the 'key' for the main.mt_chan table and so there
            // can only be 0 or 1 records.
            return (Ssutil.DbCountRows(mTableName, whereClause) == 1);
        }

        /// <summary>
        /// This method updates all of the column values of a prescribed record in the MDB TS channel table
        /// main.mt_chan.
        /// </summary>
        /// <param name="hConn"> - an open ODBC connection handle.</param>
        /// <param name="mtChan"> - a prescribed MtChan object that provides the updated field values.</param>
        /// <param name="nullInds"> - an array of ODBC nullInds associated with mtChan.</param>
        /// <returns></returns>
        public static int MtUpdateChannel(SQLHANDLE hConn, MtChan mtChan, SQLLEN[] nullInds)
        {
            //...Log2.v("\nDynMdbChannel.MtUpdateChannel(): Entry");
            ...
            update_buf = String.Format(mUPDATE, mTableName, mtChan.ToStringAsCSVequates(nullInds),
                                        mtChan.call1, mtChan.call2, mtChan.bndcde, mtChan.chid);

            //...Log2.v("\nDynMdbChannel.MtUpdateChannel(): query = " + update_buf);
            ...
            if (mMdbWriteEnabled)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    string str = String.Format("\nDynMdbChannel.MtUpdateChannel(): ERROR: SQLExecDirect failed, sqlRet = {0}, SQL query:\n{1}", sqlRet, update_buf);
                    Log2.e(str);
                    str = String.Format("mtUpdateChan04 -- Error writing Channel: {0} {1} {2} {3}",
                                  mtChan.call1, mtChan.call2, mtChan.bndcde, mtChan.chid);
            ...
            {
                Info.Text += "\r\n\r\n" + update_buf;
                //...Log2.v("\nDynMdbChannel.MtUpdateChannel(): Updating of main.mt_chan records is DISABLED.");
            }

            ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);

            //...Log2.v("\nDynMdbChannel.MtUpdateChannel(): Exit");
            return (Constant.SUCCESS);
        }

        /// <summary>
        /// Inserts a record into the MDB TS table main.mt_chan using column values prescribed 
        /// by the fields of an MtChan object.
        /// </summary>
        /// <param name="hConn"> - an open ODBC connection handle.</param>
        /// <param name="mtChan"> - a MtChan object.</param>
        ...
        public static int MtInsertChannel(SQLHANDLE hConn, MtChan mtChan, SQLLEN[] nullInd)
        {
            //...Log2.v("\n\nDynMdbChannel.MtInsertChannel(): Entry");
            ...
            if (!ODBC.IsOK(sqlRet))
            {
                Log2.e("\n\nDynMdbChannel.MtInsertChannel(): ERROR: call to SQLAllocHandle() failed.");
                return Error.ODBC_SQLALLOCHANDLE_FAILED;
            }

            //Create the SQL insert statement.
            cSQL = String.Format(mINSERT, mTableName, mtChan.ToStringAsCSV(nullInd));

            //...Log2.v("nDynMdbChannel.MeInsertChan(): SQLExecDirect():\r\n" + cSQL);
            ...
            if (mMdbWriteEnabled)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    Log2.e("\n\nDynMdbChannel.MtInsertChannel(): ERROR: a call to SQLExecDirect() failed for: " + cSQL);
                    string str = "mtInsertChan02 -- Error inserting chan";
                    Ssutil.DbGetDiagStmt(hStmt, str); ;
                    ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                    return Error.ODBC_EXECDIRECT_FAILED;
                }

                //...Log2.v("\n\nDynMdbChannel.MtInsertChannel(): successfuly inserted record into table main.mt_chan");
            }
            else
            {
                Info.Text += "\r\n\r\n" + cSQL;
                //...Log2.v("\nDynMdbChannelMtInsertChannel(): Insertion of main.mt_chan records is DISABLED.");
            }

            //This method call releases hStmt, disconnects from the DB and then releases hConn.
            ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);

            //...Log2.v("\n\nDynMdbChannel.MtInsertChannel(): Exit");
            return Constant.SUCCESS;
        }

        /// <summary>
        /// This method deletes a record with a prescribed key from the MDB
        /// TS channel table main.mt_chan.
        /// </summary>
        /// <param name="hConn"> - an open ODBC connection handle.</param>
        /// <param name="call1"> the prescribed local 'call sign' string.</param>
        /// <param name="call2"> the prescribed remote 'call sign' string.</param>
        /// <param name="bndcde"> the prescribed 'band code' string.</param>
        /// <param name="chid"> the prescribed 'channel ID'.</param>
        /// <returns></returns>
        public static int MtDeleteChannel(SQLHANDLE hConn, string call1, string call2, string bndcde, string chid)
        {
            //...Log2.v("\n\nDynMdbChannel.MtDeleteChannel(): Entry");
            ...
            string delete_buf = String.Format(mDELETE, mTableName, call1, call2, bndcde, chid);

            //...Log2.v("\nDynMdbChannel.MtDeleteChannel(): query = " + delete_buf);
            ...
            if (mMdbWriteEnabled)
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    string str = String.Format("mtDeleteChan01 -- Error deleting from main.mt_chan");
                    Ssutil.DbGetDiagStmt(hStmt, str);
                    ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);
                    Log2.e("\nDynMdbChannel.MtDeleteChannel(): ERROR: SQLExecDirect() failed for query = " + delete_buf);
                    return Error.ODBC_EXECDIRECT_FAILED;
                }
            }
            else
            {
                Info.Text += "\r\n\r\n" + delete_buf;
                //...Log2.v("\nDynMdbChannel.MtDeleteChannel(): Deletion of main.mt_chan records is DISABLED.");
            }

            ODBC.SQLFreeHandle(ODBC.SQL_HANDLE_STMT, hStmt);

            //...Log2.v("\n\nDynMdbChannel.MtDeleteChannel(): Exit");
            return 0;
        }


        /// <summary>
        /// This method updates records in the MDB <b>main.mt_chan</b> table i.a.w. the 
        /// 'GK' directives in a user's TS PDF import file; specifically, all records that
        /// have 'call1 = X' or 'call2 = X' are changed to 'call1 = Y' or 'call2 = Y' where
        /// X and Y are user-prescribed 'old' and 'new' call sign strings.
        /// </summary>
        ...
        /// <returns></returns>
        public static int MtChngUpdateChannel(SQLHANDLE hConn, FtChng ftChng, string userName, string sysDate, string sysTime, ref int chngCount)
        {
            int nRet1;
            int nRet2;
            int status;

            // Change the 'local' call signs (call1).
            nRet1 = MtChangeCallSigns(NODE.LOCAL, hConn, ftChng, userName, sysDate, sysTime, ref chngCount);

            // Change the 'remote' call signs (call2).
            nRet2 = MtChangeCallSigns(NODE.REMOTE, hConn, ftChng, userName, sysDate, sysTime, ref chngCount);

            if ((nRet1 == Constant.SUCCESS) && (nRet2 == Constant.SUCCESS))
            {
                //...Log2.v("\nDynMdbChannel.MtChngUpdateChannel(): updates of local and remote call signs SUCCEEDED.");
                status = Constant.SUCCESS;
            }
            else
            {
                Log2.e("\nDynMdbChannel.MtChngUpdateChannel(): ERROR: updates of local and remote call signs FAILED.");
        ...
        /// X and Y are user-prescribed 'old' and 'new' call sign strings.
        /// </summary>
        /// /// <param name="node"> - either LOCAL or REMOTE.</param>
        /// <param name="hConn"> - an open ODBC connection handle.</param>
        /// <param name="ftChng"> - prescribed feCCal object that prescribes X and Y.</param>
        /// <param name="userName"> - user's MICS ID.</param>
        /// <param name="sysDate"> - prescribed current system date.</param>
        ...
        /// <param name="chngCount"> - a count of the number of records that were updated.</param>
        /// <returns></returns>
        public static int MtChangeCallSigns(NODE node, SQLHANDLE hConn, FtChng ftChng, string userName, string sysDate, string sysTime, ref int chngCount)
        {
            ...
            // Create the appropriate SQL UPDATE query depending on whether
            // node is LOCAL or REMOTE.
            switch (node)
            {
                case NODE.LOCAL:
                    cSQL = String.Format(mCHNG_UPDATE_1,
                      mTableName, ftChng.newcall1, userName, sysDate, sysTime, ftChng.oldcall1);
                    break;

                case NODE.REMOTE:
                    cSQL = String.Format(mCHNG_UPDATE_2,
                      mTableName, ftChng.newcall1, userName, sysDate, sysTime, ftChng.oldcall1);

                    break;
            }

            //...Log2.v("\nDynMdbChannel.MtChangeCallSigns(): query = " + cSQL);
            ...
            if (mMdbWriteEnabled)
            {
                ...
                {
                    Log2.e("\nDynMdbChannel.MtChangeCallSigns(): ERROR: call to SQLExecDirec() failed for query =\n" + cSQL);
                    status = Constant.FAILURE;
                    Console.Write("\r\nERROR!  Could not Change MDB channel record CallSign fields.\r\n");
            ...
            {
                Info.Text += "\r\n\r\n" + cSQL;
                //...Log2.v("\nDynMdbChannel.MtChangeCallSigns(): Update of main.mt_chan records is DISABLED.");
            }
        ...
        /// <summary>
        /// Returns the number of rows found in the MDB table main.mt_chan using
        ...
        {
            //...Log2.v("\n\nDynMdbChannel.DbCountRows(): Entry");
            ...
            //...Log2.v("\r\nDynMdbChannel.DbCountRows(): cBuf = " + cBuf);
            ...
            try
            {
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    Log2.e("\r\nDynMdbChannel.DbCountRows(): ERROR: call to SQLAllocHandle() failed, sqlRet =  " + sqlRet);
                    return Error.ODBC_SQLALLOCHANDLE_FAILED;
                }
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    Log2.e("\r\nDynMdbChannel.DbCountRows(): ERROR: call to SQLExecDirect() failed, sqlRet =  " + sqlRet);
                    Log2.e("\r\nDynMdbChannel.DbCountRows(): query =  " + cBuf);
                    return Error.ODBC_EXECDIRECT_FAILED;
                }
                ...
                if (!ODBC.IsOK(sqlRet))
                {
                    Log2.e("\r\nDynMdbChannel.DbCountRows(): ERROR: call to SQLFetch() failed, sqlRet =  " + sqlRet);
            ...
            {
                Log2.e("\r\nDynMdbChannel.DbCountRows(): try-catch: ERROR: exception: " + e.Message);
            ...
            //...Log2.v("\nDynMdbChannel.DbCountRows(): Exit: nCount = " + nCount);
            return nCount;
        }

        /// <summary>
        /// This method outputs a list of MtChan objects as SELECTed using
        ...
        /// <returns></returns>
        public static int GetListOfMtChans(string searchCriteria, string orderBy, out List<MtChan> chanList)
        {
            // 'out' requirement.
            chanList = new List<MtChan>();

            MtChan mtChan;
            SQLLEN[] nullInds;
            int nRet = Constant.SUCCESS;

            int handle = DynMdbChannel.MtSelectChannel(searchCriteria, orderBy);
            if (handle < 0)
            {
                Log2.e("\n\nDynMdbChannel.GetListOfChannels(): MtSelectChannel() failed: handle = {0}", handle);
                return Constant.FAILURE;
            }

            while (DynMdbChannel.MtFetchChannel(handle, out mtChan, out nullInds) == Constant.SUCCESS)
            {
                chanList.Add(mtChan);

                //...Log2.v("\n" + String.Format("DynMdbChannel.GetListOfChannels(): mtChan = {0}, freqtx = {1}, freqrx = {2}", mtChan.KeysToString(), mtChan.freqtx, mtChan.freqrx));
            }

            DynMdbChannel.MtCloseChannel(handle);
