# Documented File: OELPlotie9.cs
**Repository Path:** `DBAccess\OELPlotie9.cs`
...
namespace DBAccess
{
                                       ...
                                       string cNote)
    {
        ...
        sw.WriteLine("<svg width='{0}' height='{1}' viewBox='{2} {3} {4} {5}'>",
                     Math.Round(nWidth),
                     Math.Round(nWidth),
                     Math.Round(-nRadius),
                                 Math.Round(-nRadius),
        ...
        sw.WriteLine("<title>{0}</title>", cTitle);
        sw.WriteLine("<desc>{0}</desc>", cNote);
        // Plot the outer circle and the coords
        sw.WriteLine("<circle cx='0' cy='0' r='{0}' " +
        ...
        for (double nAng = 0; nAng < 180; nAng += 30)
        {
            ...
            sw.WriteLine("<line x1='{0}' y1='{1}' x2='{2}' y2='{3}' />",
                         nStartx,
                         nStarty,
                         nEndx,
                         nEndy);
            //	Mark the angles
            sw.WriteLine("<text x='{0}' y='{1}' style='font-size: {2}'>{3}</text>",
                         nStartx,
                         (nAng <= 90 ? nStarty : nStarty + nMarkSize),
                         nMarkSize,
                         nAng);
            sw.WriteLine("<text x='{0}' y='{1}' text-anchor='end' style='font-size: {2}'>" +
                         "{3}</text>",
                         nEndx,
        ...
        sw.WriteLine("<text x='{0}' y='{1}' text-anchor='end' style='font-size:{2}; font-weight:bold'>" +
                     "{3}</text>",
                     nMax,
                     (-nMax),
                                 nFontSize,
    ...
    public static string plotpolarcurve(double[] aX, double[] aY, string cColour)
    {
        ...
        StringBuilder cPath = new StringBuilder("<path d='M");
    ...
    public static string plotpolarlegend(string[] aLegend, double nEndPoint)
    {
        ...
        sw.WriteLine("<rect x='{0}' y='{1}' width='{2}' height='{3}' " +
        ...
        {
            sw.WriteLine("<text x='{0}' y='{1}' style='font-size:{2};fill:{3}>{4}</text>",
                         (nEndPoint - nLineLn),
                         nStart,
                         (nLineHt * .9),
                         aColour[nInd],
    ...
    public static string plotrectback(double nWidth, double nHeight, string cTitle)
    {
        ...
        //sw.WriteLine("<svg height='{0}' width='{1}' viewBox='0 0 1000.0 {2}'>",
        //             nHeight1,
        //             nWidth1,
        //             (nRatio * 1000.0));
        sw.WriteLine("<header><h2>{0}</h2></header>", cTitle);
  
        sw.WriteLine("<svg height='{0}' width='{1}' viewBox='0 0 1000.0 {2} {3}'>",
                     nHeight1,
                     nWidth1,
                     (nRatio * 1000.0),
                     "version='1.1' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink'");
        //sw.WriteLine("{0}", cTitle);
        //sw.WriteLine("<desc>Rectangular plot of {0}</desc>", cTitle);
        sw.WriteLine("<rect x='0' y='0' height='{0}' width='1000.0' " +
                     "style='stroke:black;fill:none;stroke-width=2' />",
                     (nRatio * 1000));
        //sw.WriteLine("<title>{0}</title>", cTitle);
        //sw.WriteLine("<text x='500' y='20' text-anchor='middle' style='font-weight:bold'>" +
        //							cTitle + "</text>");
        //sw.WriteLine("<text x='500' y='35' text-anchor='middle' style='font-size:12'>" +
        //							OELSupport.standarddate() + "</text>");
        string cLogo = OELSupport.urlhead() + "images/Image4.gif";
        sw.WriteLine("<image xlink:href='" + cLogo + "' x='2' y='2' width='100' height='100' />");
        //sw.WriteLine("<text x='120' y='25' style='font-family:sans-serif;font-weight:bold'>" +
        //						 "Frequency</text>");
        //sw.WriteLine("<text x='120' y='50' style='font-family:sans-serif;font-weight:bold'>" +
        //						 "Coordination</text>");
        //sw.WriteLine("\n<text x='120' y='75' style='font-family:sans-serif;font-weight:bold'>" +
        //						 "System</text>");
        //sw.WriteLine("\n<text x='120' y='100' style='font-family:sans-serif;font-weight:bold'>" +
        //						 "Association</text>");
        sw.Close();
    ...
    public static double ploty(double nY, double nMinY, double nMaxY)
    {
            ...
            //swlogx.WriteLine("ploty-nY:" + nY.ToString() + " nMinY:" + nMinY.ToString() + " nMaxY:" + nMaxY.ToString());

            double nYval = (120 + (0.75 * nBottom)) - (((nY - nMinY) / (nMaxY - nMinY)) * (0.75 * nBottom));

            //swlogx.WriteLine("ploty-nYval:" + nYval.ToString());
        ...
        for (nInd = 0; nInd < aDesc.Length; nInd++)
        {
            ...
            sw.WriteLine("<line x1='{0}' y1='{1}' x2='{2}' y2='{3}' " +
                         "style='stroke:{4};stroke-width:{5}' />",
                                     ...
                                     (aWidth[nInd]));
            // write legend text
            sw.WriteLine("<text x='{0}' y='{1}' style='font-size:14'>{2}</text>",
                                        (xLeft + 70),
                                        (nY + 5),
                                        aDesc[nInd]);
        }

        sw.WriteLine("<rect x='{0}' y='{1}' height='{2}' width='{3}' " +
    ...
    public static string plotrectcurves(double[] aX, params object[] aArgs)
    {
        ...
        for (nInd = 0; nInd < aY.Length; nInd++)
        {
                    ...
                    nXind += nXinc)
            {
                ...
                sb.AppendFormat("{0},{1} ",
                                OELPlotie9.plotx(nXval, -180, +180),
    ...
    public static string scalex(double nX, double nStart, double nEnd)
    {
        ...
        return "\n<line x1='" + plotx(nX, nStart, nEnd).ToString() +
    ...
    public static string plotrectantx()
    {
        ...
        for (double nX = -180; nX <= 180; nX += nInc)
        {
            ...
            sw.WriteLine("\n<text x='{0}' y='{1}' " +
                         "text-anchor='middle' style='font-size:14'>{2}</text>",
                         plotx(nX, -180, +180),
                         nScaley,
                         nX);
        }
        sw.WriteLine("\n<text x='500' y='{0}' text-anchor='middle' " +
                                    " style='font-size:16'>Degrees off-axis</text>",
                                    (nScaley + 20));
        sw.Close();
    ...
    public static string plotrectanty()
    {
        ...
        for (double nY = nMinY; nY <= nMaxY; nY += nIntY)
        {
            ...
            sw.WriteLine("<text x='{0}' y='{1}' text-anchor='end' style='font-size:14'>{2}" +
                                     "</text>",
                                     (nXstart - 3),
                                     ploty(nY, 0, nMaxY),
                                     (nMaxY - nY));
        }
        sw.WriteLine("<text x='15' y='{0}' transform='rotate(90,15,{1})' " +
                     "text-anchor='middle' style='font-size:16' >" +
                                 "Discrimination (dB)</text>",
    ...
    public static string plothorizctx(double nRqco, string cInLabel)
    {
        ...
        StringWriter sw = new StringWriter(sb);
            ...
            //swlogx.WriteLine("plothorizctx-nRqco:" + nRqco.ToString() + " cInLabel:" + cInLabel);
            //swlogx.Flush();

            if (nMaxY == 0) nMaxY = 0.001;  // this blocks division by zero 
            for (double nY = nMinY; (nRqco >= 0) ? nY <= nMaxY : nY >= nMaxY; nY += nIntY)
            {
                //swlogx.WriteLine("plothorizctx-nY:" + nY.ToString() + " nRqco:" + nRqco.ToString() + " nMinY:" + nMinY.ToString() + " nMaxY:" + nMaxY.ToString());
                //swlogx.Flush();

                if (nY != nMinY && nY != nMaxY)
                {
                    //swlogx.WriteLine("plothorizctx-before scaley");
                    //swlogx.Flush();

                    sw.WriteLine(scaley(nY, nMinY, nMaxY));
                }

                //swlogx.WriteLine("plothorizctx-before ploty:plotyval:" + ploty(nY, nMinY, nMaxY).ToString());
                //swlogx.Flush();

                sw.WriteLine("<text x='{0}' y='{1}' " +
                             "text-anchor='end' style='font-size:14'>{2}</text>",
                             (nXstart - 3),
                             ploty(nY, nMinY, nMaxY),
                             nY);
            }

        // write vertical units legend
        sw.WriteLine("<text x='15' y='{0}' transform='rotate(90,15,{1})' " +
                     "text-anchor='middle' style='font-size:16' >{2}</text>",
                     ploty(nMaxY / 2, 0, nMaxY),
                     ploty(nMaxY / 2, 0, nMaxY),
                     cLabel);
        sw.Close();
    ...
    {
        //swlogx.WriteLine("plotrectctxx-nMinY:" + nMinY.ToString() + " nMaxY" + nMaxY.ToString());
        int nInd;
        ...
        //swlogx.WriteLine("plotrectctxx-nLow:" + nLow.ToString() + " nNumber" + nNumber.ToString());
        StringBuilder sb = new StringBuilder();
        StringWriter sw = new StringWriter(sb);
        ...
        {
                //swlogx.WriteLine("plotrectctxx-X Scale: (" + nX + ", " + nXval + ")");
                //	Put up a log series of coordinates
                for (nInd = 0; nInd < aLineMults.Length; nInd++)
                {
                    //swlogx.WriteLine("plotrectctxx-aLineMults.Length: " + aLineMults.Length.ToString() + ", nInd:" + nInd.ToString());
                    // The following i was added Feb 1, 2019 by BA
                    // nX = 4 indicates axis value from 1000 to 10000  so first entry (nX=0)  must be printed
                    // any cases where nX=4 and nX>0 shuld be suppressed as these represent data outside the rectangle
                    if (!(nX == nNumber && nInd > 0))
                    {
                        //swlogx.WriteLine("plotrectctxx-scalex(nX + aLineMults[nInd]: " + (nX + aLineMults[nInd]).ToString() + ", nNumber:" + nNumber.ToString());
                        sw.WriteLine(scalex(nX + aLineMults[nInd], 0, nNumber));
                    }
                    //swlogx.WriteLine("plotrectctxx-Math.Abs(nXval - 1000): " + Math.Abs(nXval - 1000).ToString() + ", nX:" + nX.ToString() + ", aLinePrint[nInd]:"  + aLinePrint[nInd].ToString());

                    if (Math.Abs(nXval - 1000) < .1 || (nX > 0 && nInd == 0) || aLinePrint[nInd] < 0)
                {
                    //    if (aLinePrint[nInd] != 0)
                    // The above if replaced Feb 1, 2019 by BA
                    // nX = 4 indicates axis value from 1000 to 10000  so first entry (nX=0)  must be printed
                    // any cases where nX=4 and nX>0 shuld be suppressed as these represent data outside the rectangle
                    if (aLinePrint[nInd] != 0 && !(nX == nNumber && nInd > 0))
                    {
                            //swlogx.WriteLine("plotrectctxx-nX + aLineMults[nInd]: " + (nX + aLineMults[nInd]).ToString() + ", nNumber:" + nNumber.ToString());
                            //swlogx.WriteLine("plotrectctxx-nScaley: (" + nScaley.ToString() + ", nXval * aLineMarks[nInd] / 1000:" + (nXval * aLineMarks[nInd] / 1000).ToString());
        ...
        sw.WriteLine("<text x='500' y='{0}' text-anchor='middle' " +
                                 " style='font-size:16'>Frequency Separation (MHz)</text>",
                                 (nScaley + 20));
        sw.Close();
    ...
    public static string plotytick(double nY, int nColour)
    {
        ...
        return "\n<circle cx='" + plotx(0, 0, 1).ToString() +
    ...
    public static string plotctxcurves(double[] aFsep, double[] aRQ, string cRQ, string cLabel)
    {
        ...
        //string logfile = "D:\\extractlogs\\plotpowerfilter.txt";  // log X scale
                                                               // open logfile
        //swlogx = new StreamWriter(logfile, false);
        //swlogx.WriteLine("plotctxcurves-nMinY:" + nMinY.ToString() + " nMaxY:" + nMaxY.ToString());
        try
        { 
            ...
            //swlogx.WriteLine("plotctxcurves-1nMinY:" + nMinY.ToString() + " nMaxY:" + nMaxY.ToString());
            ...
            nMinY = aScale[0];
                ...
                //swlogx.WriteLine("plotctxcurves-2nMinY:" + nMinY.ToString() + " nMaxY:" + nMaxY.ToString());
                //swlogx.Flush(); 
                //	y-axis scale
                sw.WriteLine(plothorizctx(+1.0, cLabel));
                //swlogx.WriteLine("plotctxcurves-3nMinY:" + nMinY.ToString() + " nMaxY:" + nMaxY.ToString());
                //swlogx.Flush();
            ...
            // add vertical lines along X-axis
