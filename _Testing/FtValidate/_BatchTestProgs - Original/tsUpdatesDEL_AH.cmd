...
set LogFilePath=d:\Users\ahulme\temp\deltsupdates%Tag%.txt
...
echo %PDFdir%\delsadd.txt           >> tempFile.txt
echo %PDFdir%\delowsadd.txt         >> tempFile.txt
echo %PDFdir%\deldbleadd.txt        >> tempFile.txt
echo %PDFdir%\delowdadd.txt         >> tempFile.txt
echo %PDFdir%\delregadd.txt         >> tempFile.txt
...
for /f %%G IN (tempFile.txt) DO (
	...
	REM the FtValidate argument -m:-1 that is used in all the other test .cmd files
	REM is absent here. Consequently, FtValidate performs a HiLoCheck using the
    REM default distance of 0.22Km (7 Seconds of arc).	
	
	%FtImportDir%\ftImport                  %DBName% %ProjCode% -f    %%~nG %%G  >> %LogFilePath%
	%FtValidateDir%\ftValidate %Spoofing%   %DBName% %ProjCode%       %%~nG      >> %LogFilePath%
