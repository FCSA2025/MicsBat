...
-- so as to avoid a {call1, call2, '6A', anum} key set CLASH.
-----------------------------------------------------------------------------------------
...
-- Mark as 'affected' only those antenna records which have an associated channel whose (tx, rx) is within 6A.
UPDATE hulme.temp1
	SET affected = 1
	FROM hulme.temp1 AS T1
	INNER JOIN hulme.mt_chan AS C ON ((C.call1 = T1.call1) AND (C.call2 = T1.call2) AND (C.bndcde = T1.bndcde))
	WHERE   (hulme.IsInRangeFloatTxRx(5850000, 6425000, C.freqtx, C.freqrx) = 1)
...
--SELECT call1, call2, bndcde, anum, ause FROM hulme.mt_ante WHERE bndcde='6B'
...
UPDATE hulme.mt_chan SET antnumbtx1 = T.newAnum
	...
	WHERE (C.antnumbtx1 = T.anum) AND (hulme.IsInRangeFloatTxRx(5850000, 6425000, freqtx, freqrx) = 1)

--SELECT call1, call2, bndcde, chid, antnumbtx1, antnumbtx2, antnumbrx1, antnumbrx2, antnumbrx3, freqtx, freqrx, hulme.IsInRangeFloatTxRx(5850000, 6425000, freqtx, freqrx) AS inBand6A FROM hulme.mt_chan
--SELECT * FROM hulme.temp3

UPDATE hulme.mt_chan SET antnumbtx2 = T.newAnum
	FROM hulme.mt_chan AS C
	INNER JOIN hulme.temp3 AS T ON (T.call1=C.call1 AND T.call2=C.call2 AND T.bndcde=C.bndcde)
	WHERE (C.antnumbtx2 = T.anum) AND (hulme.IsInRangeFloatTxRx(5850000, 6425000, freqtx, freqrx) = 1)
...
UPDATE hulme.mt_chan SET antnumbrx1 = T.newAnum
	FROM hulme.mt_chan AS C
	INNER JOIN hulme.temp3 AS T ON (T.call1=C.call1 AND T.call2=C.call2 AND T.bndcde=C.bndcde)
	WHERE (C.antnumbrx1 = T.anum) AND (hulme.IsInRangeFloatTxRx(5850000, 6425000, freqtx, freqrx) = 1)
...
UPDATE hulme.mt_chan SET antnumbrx2 = T.newAnum
	FROM hulme.mt_chan AS C
	INNER JOIN hulme.temp3 AS T ON (T.call1=C.call1 AND T.call2=C.call2 AND T.bndcde=C.bndcde)
	WHERE (C.antnumbrx2 = T.anum) AND (hulme.IsInRangeFloatTxRx(5850000, 6425000, freqtx, freqrx) = 1)
...
UPDATE hulme.mt_chan SET antnumbrx3 = T.newAnum
	FROM hulme.mt_chan AS C
	INNER JOIN hulme.temp3 AS T ON (T.call1=C.call1 AND T.call2=C.call2 AND T.bndcde=C.bndcde)
	WHERE (C.antnumbrx3 = T.anum) AND (hulme.IsInRangeFloatTxRx(5850000, 6425000, freqtx, freqrx) = 1)

--SELECT call1, call2, bndcde, chid, antnumbtx1, antnumbtx2, antnumbrx1, antnumbrx2, antnumbrx3, freqtx, freqrx, hulme.IsInRangeFloatTxRx(5850000, 6425000, freqtx, freqrx) AS inBand6A FROM hulme.mt_chan
--SELECT * FROM hulme.temp3

---------------------------------------------------------------------------------------
-- We are now ready to perform the updates to the mt_ante to change bndcde '6B' to '6A' 
...
UPDATE hulme.mt_ante 
	...
	WHERE (A.bndcde='6B') AND (isIn6A = 1)

--SELECT call1, call2, bndcde, anum FROM hulme.mt_ante

----------------------------------------------------------------------------------------------------------
-- For those mt_chan records where changing bndcde from '6B' to '6A' would result in a duplicate key CLASH
...
UPDATE CY
	...
	WHERE (hulme.IsInRangeFloatTxRx(5850000, 6425000, CY.freqtx, CY.freqrx) = 1) 

--SELECT call1, call2, bndcde, chid, antnumbtx1, antnumbtx2, antnumbrx1, antnumbrx2, antnumbrx3, freqtx, freqrx, hulme.IsInRangeFloatTxRx(5850000, 6425000, freqtx, freqrx) AS inBand6A FROM hulme.mt_chan WHERE chid LIKE 'X%'
--SELECT * FROM hulme.temp3

---------------------------------------------------------------------------------------
-- We are now ready to perform the updates to the mt_chan to change bndcde '6B' to '6A' 
-- because we have ensured that there will be no key set CLASHes with existing records.
---------------------------------------------------------------------------------------
UPDATE hulme.mt_chan 
	SET bndcde = '6A' 
	FROM hulme.mt_chan
	WHERE (bndcde='6B') AND (hulme.IsInRangeFloatTxRx(5850000, 6425000, freqtx, freqrx) = 1)
...
SELECT 'Reassignment of MDB TS records using bndcde 6B to 6A has been completed.'