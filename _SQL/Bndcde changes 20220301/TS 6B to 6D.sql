...
-- to change records whose bndcde is '6B' to be '6D'.
--
...
-- the correct frequency range for the 'target' bndcde 6D.
--
...
--
--      the update of this record to have key set         'mycall1', 'mycall2', '6D', 24
...
-- so as to avoid a {call1, call2, '6D', anum} key set CLASH.
-----------------------------------------------------------------------------------------
...
SELECT AY.call1, AY.call2, AY.bndcde, AY.anum, 0 AS affected, 0 AS newAnum
	...
	INNER JOIN hulme.mt_ante AS AY ON (AX.call1=AY.call1 AND AX.call2=AY.call2 AND AX.bndcde='6D' AND AY.bndcde='6B' AND AX.anum=AY.anum)

--SELECT * FROM hulme.temp1

-- Mark as 'affected' only those antenna records which have an associated channel whose (tx, rx) is within 6D.
UPDATE hulme.temp1
	SET affected = 1
	FROM hulme.temp1 AS T1
	INNER JOIN hulme.mt_chan AS C ON ((C.call1 = T1.call1) AND (C.call2 = T1.call2) AND (C.bndcde = T1.bndcde))
	WHERE   (hulme.IsInRangeFloatTxRx(6425000, 6930000, C.freqtx, C.freqrx) = 1)
		AND ((T1.anum = C.antnumbtx1) OR (T1.anum = C.antnumbtx2) OR (T1.anum = C.antnumbrx1) OR (T1.anum = C.antnumbrx2) OR (T1.anum = C.antnumbrx3))

--SELECT * FROM hulme.temp1

-- For existing '6D' ante records, identify all the existing anums for which a clash could occur.
...
SELECT A.call1 AS call1, A.call2 AS call2, T1.bndcde AS bndcde, T1.anum AS anum, A.anum AS anumX
    INTO hulme.temp2
    FROM hulme.temp1 AS T1
	INNER JOIN hulme.mt_ante AS A ON (A.call1=T1.call1 AND A.call2=T1.call2 AND A.bndcde='6D')
	WHERE (T1.bndcde='6B') AND (T1.affected = 1)

--SELECT * FROM hulme.temp2

-- Calculate the maximum (+1) of the '6D' anums and set this as the anum of the '6B' records.
...
UPDATE hulme.mt_chan SET antnumbtx1 = T.newAnum
	...
	WHERE (C.antnumbtx1 = T.anum) AND (hulme.IsInRangeFloatTxRx(6425000, 6930000, freqtx, freqrx) = 1)

--SELECT call1, call2, bndcde, chid, antnumbtx1, antnumbtx2, antnumbrx1, antnumbrx2, antnumbrx3, freqtx, freqrx, hulme.IsInRangeFloatTxRx(6425000, 6930000, freqtx, freqrx) AS inBand6D FROM hulme.mt_chan
--SELECT * FROM hulme.temp3

UPDATE hulme.mt_chan SET antnumbtx2 = T.newAnum
	FROM hulme.mt_chan AS C
	INNER JOIN hulme.temp3 AS T ON (T.call1=C.call1 AND T.call2=C.call2 AND T.bndcde=C.bndcde)
	WHERE (C.antnumbtx2 = T.anum) AND (hulme.IsInRangeFloatTxRx(6425000, 6930000, freqtx, freqrx) = 1)
...
UPDATE hulme.mt_chan SET antnumbrx1 = T.newAnum
	FROM hulme.mt_chan AS C
	INNER JOIN hulme.temp3 AS T ON (T.call1=C.call1 AND T.call2=C.call2 AND T.bndcde=C.bndcde)
	WHERE (C.antnumbrx1 = T.anum) AND (hulme.IsInRangeFloatTxRx(6425000, 6930000, freqtx, freqrx) = 1)
...
UPDATE hulme.mt_chan SET antnumbrx2 = T.newAnum
	FROM hulme.mt_chan AS C
	INNER JOIN hulme.temp3 AS T ON (T.call1=C.call1 AND T.call2=C.call2 AND T.bndcde=C.bndcde)
	WHERE (C.antnumbrx2 = T.anum) AND (hulme.IsInRangeFloatTxRx(6425000, 6930000, freqtx, freqrx) = 1)
...
UPDATE hulme.mt_chan SET antnumbrx3 = T.newAnum
	FROM hulme.mt_chan AS C
	INNER JOIN hulme.temp3 AS T ON (T.call1=C.call1 AND T.call2=C.call2 AND T.bndcde=C.bndcde)
	WHERE (C.antnumbrx3 = T.anum) AND (hulme.IsInRangeFloatTxRx(6425000, 6930000, freqtx, freqrx) = 1)

--SELECT call1, call2, bndcde, chid, antnumbtx1, antnumbtx2, antnumbrx1, antnumbrx2, antnumbrx3, freqtx, freqrx, hulme.IsInRangeFloatTxRx(6425000, 6930000, freqtx, freqrx) AS inBand6D FROM hulme.mt_chan
--SELECT * FROM hulme.temp3

---------------------------------------------------------------------------------------
-- We are now ready to perform the updates to the mt_ante to change bndcde '6B' to '6D' 
...
-- their bndcde changed from 6B to 6D.
...
UPDATE hulme.mt_ante 
	...
	WHERE (A.bndcde='6B') AND (isIn6D = 1)

--SELECT call1, call2, bndcde, anum FROM hulme.mt_ante
--SELECT * FROM hulme.temp3

----------------------------------------------------------------------------------------------------------
-- For those mt_chan records where changing bndcde from '6B' to '6D' would result in a duplicate key CLASH
...
UPDATE CY
	...
	INNER JOIN hulme.mt_chan AS CY ON (CX.call1=CY.call1 AND CX.call2=CY.call2 AND CX.bndcde='6D' AND CY.bndcde='6B' AND CX.chid=CY.chid)
	WHERE (hulme.IsInRangeFloatTxRx(6425000, 6930000, CY.freqtx, CY.freqrx) = 1) 

--SELECT call1, call2, bndcde, chid, antnumbtx1, antnumbtx2, antnumbrx1, antnumbrx2, antnumbrx3, freqtx, freqrx, hulme.IsInRangeFloatTxRx(6425000, 6930000, freqtx, freqrx) AS inBand6D FROM hulme.mt_chan
--SELECT * FROM hulme.temp3

---------------------------------------------------------------------------------------
-- We are now ready to perform the updates to the mt_chan to change bndcde '6B' to '6D' 
-- because we have ensured that there will be no key set CLASHes with existing records.
---------------------------------------------------------------------------------------
UPDATE hulme.mt_chan 
	SET bndcde = '6D' 
	FROM hulme.mt_chan
	WHERE (bndcde='6B') AND (hulme.IsInRangeFloatTxRx(6425000, 6930000, freqtx, freqrx) = 1)
...
GO
SELECT 'Reassignment of MDB TS records using bndcde 6B to 6D has been completed.'