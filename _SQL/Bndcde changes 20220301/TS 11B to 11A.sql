...
-- to change records whose bndcde is '11B' to be '11A'.
--
...
--
--      mt_ante record key set (call1,call2,bndce,anum):  'mycall1', 'mycall2', '11B', 24
--
--      the update of this record to have key set         'mycall1', 'mycall2', '11A', 24
...
-----------------------------------------------------------------------------------------
-- Create a table that captures the '11B' antenna records whose anum needs to be changed
...
-- For existing '11A' ante records, identify all the existing anums for which a clash could occur.
...
SELECT A.call1 AS call1, A.call2 AS call2, T1.bndcde AS bndcde, T1.anum AS anum, A.anum AS anumX
	...
	INNER JOIN hulme.mt_ante AS A ON (A.call1=T1.call1 AND A.call2=T1.call2 AND A.bndcde='11A')
	WHERE T1.bndcde='11B'

-- Calculate the maximum (+1) of the '11A' anums and set this as the anum of the '11B' records.
...
UPDATE hulme.mt_ante
	...
	INNER JOIN hulme.temp3 AS T ON (T.call1=A.call1 AND T.call2=A.call2 AND T.bndcde='11B' AND T.anum=A.anum)
	WHERE A.bndcde='11B'
...
UPDATE hulme.mt_chan
	...
	INNER JOIN hulme.temp3 AS T ON (T.call1=C.call1 AND T.call2=C.call2 AND T.bndcde='11B')
	WHERE C.bndcde='11B'
...
UPDATE CY
	...
	INNER JOIN hulme.mt_chan AS CY ON (CX.call1=CY.call1 AND CX.call2=CY.call2 AND CX.bndcde='11A' AND CY.bndcde='11B' AND CX.chid=CY.chid)

-------------------------------------------------------------------------------------
-- We are now ready to perform the 'simple' updates to the mt_ante and mt_chan tables
-- to change bndcde '11B' to '11A' because we have now ensured that there will be no
-- key set clashes with existing records.
---------------------------------------------------------------------------
UPDATE hulme.mt_ante SET bndcde = '11A' WHERE bndcde = '11B'

UPDATE hulme.mt_chan SET bndcde = '11A' WHERE bndcde = '11B'
...
SELECT 'Reassignment of MDB TS records using bndcde 11B to 11A has been completed.'

-----------------------------------
-- Inspect and approve the results.
-----------------------------------
/*
SELECT * FROM hulme.mt_ante WHERE bndcde = '11B'
...
SELECT A.call1, A.call2, A.bndcde, A.anum, C.chid, C.antnumbtx1, C.antnumbtx2, C.antnumbrx1, C.antnumbrx2, C.antnumbrx3
	...
	WHERE C.bndcde='11A' AND ((antnumbtx1 IS NOT NULL) OR (antnumbtx2 IS NOT NULL) OR (antnumbrx1 IS NOT NULL) OR (antnumbrx2 IS NOT NULL) OR (antnumbrx3 IS NOT NULL))--WHERE A.call1='WPVL20301' AND A.call2='WPVL20303'
...
SELECT AY.call1, AY.call2, AY.bndcde, AY.anum, AX.bndcde, AX.anum
    FROM hulme.mt_ante AS AX
	INNER JOIN hulme.mt_ante AS AY ON (AX.call1=AY.call1 AND AX.call2=AY.call2 AND AX.bndcde='11A' AND AY.bndcde='11B')
