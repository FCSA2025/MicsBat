...
GO
-- Returns 1 (true) if the distance in Kilometres given lat/long inputs in which the first pair
-- is in centi-seconds and the second pair in degrees is less than a prescribed distance (kms).
-- The algorithm follows that used in the MICS# AxDistan() function, to give the same result. */
ALTER function [hulme].[distance_less_than_cs_deg](@lat1 as bigint, @lng1 as bigint, @lat2 as float, @lng2 as float, @radius_kms as float)
returns BIT
begin
